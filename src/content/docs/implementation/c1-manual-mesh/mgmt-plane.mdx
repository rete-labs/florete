...
Both vertex configs use a unified `workloads` shape — one entry per local principal/target. Each workload has zero or more `io` channels, and **each channel's direction is intrinsic to its `kind`** (matching C0 scope's "Inbound/Outbound" usage):

- **inbound** kind (`socks5`) — the workload connects *into* flor here; flor originates outgoing connections on its behalf. Workload acts as a **principal**.
      ...
      "members": [
        {
          ...
          "via": { "adapter": "wire", "udp_peer": "5.6.7.8:4433" }
        }
      ]
    }
  ],
...
- [ ] `flor vertex run --rete <scope> --vertex <name>` daemon entry point (reads mgmt + mesh ctrl configs by name; resolves identities against the rete root)
