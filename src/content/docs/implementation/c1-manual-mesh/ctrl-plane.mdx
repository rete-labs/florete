...
- **Envelope** — same shape as the mgmt envelope (see [C0's spec](../../c0-tended-tunnels/mgmt-plane/validate-and-compile#compile-step)) with three additions: `plane: "ctrl"`, the `vertex` claim, and `obeys_mgmt_version`. The last pins which mgmt artifact this ctrl decision was made against; agents reject ctrl whose `obeys_mgmt_version` is older than the mgmt they currently hold (rollback-attack defense).
...
- **`label_bindings`** — binds each accepted (initiator, target) flow from the mgmt allow lists to a per-link label: an **ingress** binding is `in: {link, label}` → local delivery; an **egress** binding is local origin → `out: {link, label}` (the first hop). Each binding's (scope, target_spiffe_id, principal) **must** appear in the corresponding `ingress.allow` or `egress.allow` flat list; this is the local verifier's check.
