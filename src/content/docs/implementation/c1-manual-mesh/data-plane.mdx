...
- Mesh-vertex with QUIC endpoint for transport service needs UDP socket to work on: that's natural for any implementation because QUIC runs over UDP sockets. Moreover, almost any transport endpoint will require some sort of a datagram socket - e.g. TCP over IP, or DTLS over UDP, or WireGuard over UDP.
...
- we get real socket structure for the mesh-vertex to use
...
- Mesh-vertex must use some datagram socket to communicate with link-vertex
...
- Recursiveness: datagram socket used to communicate with link vertex should be reusable by mesh-vertex users, including interrete vertex
...
- ForwardingEngine:
   - (mesh & others) forwarding by MPLS-inspired labels
...
Here we have `flor` binary that can be used recursively. I.e. normally we'd have 2 of them running: `flor --mesh ` and `flor --link`. Inbound/outbound interface for them will be common and allow TCP and UDP. Recursive case will be UDP-based.
...
First: how to add 3rd party links in this design? They cannot be drop-in replacements for `flor --link`. So here design must follow Non-recursive case with such links added to `flor --mesh`, which will then run without `flor --link`. Or we'd have `flor --link` as a middle-man/shim between `flor --mesh` and 3rd party links.
...
So third parties won't provide Florete Link Vertex API to Flor! They will be plugged-in by entirely custom adapters using whatever they need to work as links for mesh layer.
...
- Alice wants to TCP-connect to Bob, which is published as e.g. "bob.rete"
- Alice requests "connect bob.rete" command in some form
...
Regarding L7 proxy (HTTP), it would be nice to have for rete scenarios for load-balancing later - just like Envoy in Kubernetes. In their current design though L7 isn't per-node, but rather some instances in the rete (how they're called?). In our case it can be distributed service with some deploy policy (not per node).
...
- Bob is a TCP server
- The Outbound publishes it in the mesh layer as "bob.rete" service
...
The main idea of splitting mesh and link vertices can be implemented in a few ways. Let's design from ground up here, from link to mesh because we'll need to support third party links later.
...
The mesh process needs to handle this complexity via adaptor pattern.

Recursiveness: the mesh process must be able to work on top of another mesh process.
...
- mesh/recursive mode: uses supported link providers; initially we'll have only our own link provider, "flor-link"
...
1. `flor(mesh) => flor(link)/3rd-party-link`
2. `flor(mesh) => flor(link) => flor-link/3rd-party-link`
