...
C0 has only user and service principals. C1 adds **cluster vertex** identities — one per node per Florete cluster — used for inter-vertex mTLS on mesh links. A node joining multiple clusters runs multiple vertex identities. Everything else about the identity model (CA, enrollment bundles, revocation) carries over unchanged.
