...
The `.flor/compiled/` tree holds **`plane: "mgmt"` artifacts only.** Ctrl artifacts (`plane: "ctrl"`, B1+) never live in the repo: they're CP-generated, fast-changing, and unsigned — wrong lifecycle for git. They live only in the coordination server's runtime store and in the agent's local state cache. The repo's job is operator-authored signed state and its audit log; that scope doesn't change when CP lands.
