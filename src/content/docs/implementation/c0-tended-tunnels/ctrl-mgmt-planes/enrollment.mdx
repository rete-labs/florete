...
5. **Bootstrap the operator's own machine** (the first Florete participant): issue `florctl issue-bundle --node fyodor-laptop`, copy its `fyodor-laptop.json` compiled artifact locally (same pre-Florete route, since publish isn't available yet), install the bundle + artifact, start `flor agent run`. The operator now runs flor as the `fyodor` user principal with `role: operator`, and `florctl` auto-discovers the operator's SOCKS5 port via the local agent socket (see [florctl local wiring](#florctl-local-wiring) below).
