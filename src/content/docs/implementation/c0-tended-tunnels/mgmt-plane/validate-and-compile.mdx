...
`florctl validate`/`compile` first **discover and merge** the source: starting from `rete.yaml`, they recursively glob `*.yaml`/`*.yml` (skipping `.flor/`, `certs/`, `enrollment.log`, dotfiles; honoring any `include`/`exclude` in `rete.yaml`), then union same-kind collections into one logical view. File layout is operator-chosen (see [Source Layout](../source-layout)); the rules below run on the merged view.
