...
- `schema_version` — `major.minor` of the compiled-artifact contract. Every consumer gates it fail-closed: it accepts its own major at any older-or-equal minor and rejects anything newer with an explicit upgrade error; the producer stamps each artifact with the lowest minor its content actually uses. See [ADR-0012](/docs/implementation/adr/0012-version-artifact-contract) for the contract-wide versioning policy.
