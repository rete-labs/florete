...
- `signature` — `{ alg, cert, value }`: the operator's signature over the canonical-form envelope minus the `signature` field itself, and the **certificate of the signer that produced it**. There is no `key_id`; the certificate carries the SPIFFE ID in its SAN, and a verifier reads the identity from there. The signer is a **management-plane** principal — a sign-only identity issued independently from any user entry via `retectl id issue --kind management-plane`, whose key never leaves operator hardware. See [ADR-0005](/docs/implementation/adr/0005-use-spiffe-identities) for the kind separation and [ADR-0015](/docs/implementation/adr/0015-anchor-signer-trust-in-root-ca) for why the certificate travels with the signature.
