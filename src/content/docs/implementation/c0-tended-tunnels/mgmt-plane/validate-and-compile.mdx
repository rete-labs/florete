...
**Artifacts carry no filesystem references.** Identity material is referenced by SPIFFE ID only and materialized on the node by the per-scope identity store (`identity/ca.crt` under the scope root, `identity/certs/<leaf>.crt` / `.key` per principal — see [Agent § Artifact handling](../agent#artifact-handling)); local sockets and paths are derived on the node from spawn-injected roots and names. This keeps artifacts fully portable: the same compiled JSON is valid whatever the operator named the rete scope locally, wherever the node's roots live, and on any OS. The repo's `certs/management-planes/<name>.crt` shape is for human/audit organisation; mgmt-signer pubkeys reach nodes inside `agent.json`'s `trust.authorized_mgmt_signers`, never as on-disk files.
