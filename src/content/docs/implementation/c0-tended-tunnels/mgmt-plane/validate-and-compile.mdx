...
6. **Management-node integrity** — services named `config-server` and `config-publisher` must exist in groups `control-plane` and `control-plane-write` respectively, both hosted on the same node (the management node) with a publicly-reachable link-vertex (a `kind: link, type: quic` entry with an `address`). If missing or malformed, validator fails with a clear message rather than silently producing artifacts that can't sync.
...
- `signature` — operator signature over the canonical-form envelope minus the `signature` field itself. `key_id` pins which operator key produced it; the agent verifies against the operator public key delivered alongside `ca.crt` at enrollment. The signing key never leaves operator hardware. Algorithm details, key rotation, and emergency-revocation mechanics are not designed in C0/C1 — single operator key with file-backed storage is sufficient at this scale.
