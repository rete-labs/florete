...
7. **Reserved-name protection** — `node` and `operator` (roles), `config-read` and `config-write` (groups) must be present in the YAML with their canonical `allow` / member definitions. Validator fails if they're missing or redefined with different structure. The compiler never writes YAML — these live in the repo as part of the initial cluster template.
...
- `authorized_mgmt_signers` — the cluster's currently-authorized mgmt-envelope signer set, derived by the compiler from `cluster.yaml`'s `signers.mgmt.keys` list. Each entry pairs the signer's SPIFFE URI with its ed25519 pubkey. Agents accept an artifact only if `signature.key_id` matches one of these entries and the signature verifies against the corresponding pubkey. The bootstrap snapshot of this list also ships in the enrollment bundle; thereafter the list lives in every fetched artifact, so signer rotation propagates via the normal `flor agent sync` flow (operator adds the new signer to `cluster.yaml`, publishes, agents sync, then operator switches to signing with the new key — standard overlap-window rotation).
