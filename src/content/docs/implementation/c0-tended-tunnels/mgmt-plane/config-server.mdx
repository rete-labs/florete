...
- `config-server: GET /artifact/<node>?version=<n>` — returns the node's compiled directory (an `agent.json` + the vertex configs under `vertices/`) packaged as a single response if the caller passed the `control-plane` ingress gate. (No path/identity cross-check in C0; see note above.)
- `config-publisher: POST /tree` — accepts a new compiled tree from `florctl`. Only reachable by callers with `operator` role.

The caller of `GET /artifact/<node>` is the node's `flor-agent`, acting as the `nodes/<node>` principal via the local vertex's SOCKS5 listener. C0 doesn't need anything more elaborate — no streaming, no deltas, no watch. Agents poll on their own cadence via `flor agent sync`.
