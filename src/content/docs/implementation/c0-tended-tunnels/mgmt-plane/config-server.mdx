...
roles:
  ...
  operator: { allow: [config-write] }   # manually assigned in users.yaml
```
...
- `node`, `operator` (roles), `config-read`, `config-write` (groups), and the `config-server` / `config-publisher` services are all **reserved names**. Validator fails if they're missing or redefined with different structure. Florete compiler never *writes* YAML — these live in the repo as part of the initial cluster template.
- **Role assignment** is split: the `node` role is auto-assigned by the compiler to every `node/` principal (so each node's compiled artifact has the right egress row to reach `config-server`). The `operator` role is assigned manually in `users.yaml` (`fyodor: { role: operator }`) — the validator rejects the cluster if no user has it (otherwise nothing can publish).
