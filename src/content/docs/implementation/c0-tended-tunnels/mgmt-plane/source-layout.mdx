...
Each node's compiled directory holds an `agent.json` (read by `flor agent run`) and a `flor.json` (read by the single `flor-vertex` instance the agent supervises in C0), flat beside each other. C1 adds a second vertex artifact; the agent.json shape is stable across both.
