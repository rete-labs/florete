...
**The three must nest: no certificate may outlive the root that issued it.** The slots are independent, so a leaf minted late in the root's life can carry a `notAfter` past its anchor's — and the chain then fails at the *root's* expiry while the leaf still looks valid. The validator refuses that at `compile` and at `issue-bundle` rather than warning, because the failure is fleet-wide and arrives after the material is couriered and installed. See [Enrollment § Decision Log](../enrollment#decision-log), ENRL-C0-25.
