...
| Filesystem directories holding collections of named instances | **plural** collection name | `certs/management-planes/primary.crt`, `artifacts/mgmt/sets/<v>/`, `<state-dir>/retes/<scope>/` |
...
Each node's compiled directory holds an `agent.json` (read by `flor agent run`) and a `flor.json` (read by the single `flor vertex run` instance the agent supervises in C0), flat beside each other. C1 adds a second vertex artifact; the agent.json shape is stable across both.
