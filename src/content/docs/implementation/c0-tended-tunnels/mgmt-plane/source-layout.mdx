...
rete:
  ...
  signers:
    mgmt:
      ...
      principals:
        - { name: primary }                              # resolved in retectl's own tree
...
The **mgmt-envelope signing principal** is a separate identity, issued on operator hardware via `retectl id issue --kind management-plane --name primary`. Its name (`primary` by convention) is independent of any user — the keypair belongs to the *rete*'s signing authority, not to a specific person — and it names a **slot**, since a node accepts a signer on its certificate's chain to the anchors and never on its name. The cert is sign-only (`keyUsage: digitalSignature`, no `extKeyUsage`) so it cannot terminate TLS. Only the certificate is shipped to nodes, inside the `signature` of each envelope it signed, and is verified against the anchors delivered at enrollment. Issuing or rotating this key is an explicit operator step, never derived from `users.yaml`.
