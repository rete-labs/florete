...
The `*.yaml` filenames above are a **recommended default** (scaffolded by `florctl init`), not a fixed schema. The facade is a *whole-view* document — the operator authors the entire mgmt view and the compiler reads it as a whole — so **file boundaries are cosmetic**. Operators may keep everything in one file, or split by team/domain (`services/payments.yaml`, `services/web.yaml`), or group however suits the rete. See [ADR-0009](../../../../contributing/adr/0009-config-compiled-facade) for why the facade is compose-style rather than a collection of Kubernetes-style objects.
...
- **Discovery.** `florctl compile`/`validate` find `rete.yaml` (the fixed anchor — it names the rete and holds CA/signer policy), then recursively glob `*.yaml`/`*.yml` under the repo, skipping `.flor/`, `certs/`, `enrollment.log`, and dotfiles. `rete.yaml` may carry a `source:` block with `include`/`exclude` globs to override discovery precisely (useful in a shared repo); `florctl compile -f <file|glob>…` overrides discovery entirely for partial/CI compiles.
...
The **mgmt-envelope signing principal** is a separate identity, issued out-of-band on operator hardware via `florctl ca sign --kind management-plane --name primary`. Its name (`primary` by convention, mirroring `control-plane/primary` in C1) is independent of any user — the keypair belongs to the *rete*'s signing authority, not to a specific person. The cert is sign-only (`keyUsage: digitalSignature`, no `extKeyUsage`) so it cannot terminate TLS. Only the public cert is shipped to nodes, alongside `ca.crt` at enrollment. Issuing or rotating this key is an explicit operator step, never derived from `users.yaml`.
