...
- **Install root**: `<root>/retes/<scope>/` (user mode `~/.flor`, system mode `/var/lib/flor` — see [Agent § Node layout](agent#node-layout)) — the identity store (`ca.crt`, `certs/`), immutable artifact sets under `artifacts/`, durable agent state under `state/`.
