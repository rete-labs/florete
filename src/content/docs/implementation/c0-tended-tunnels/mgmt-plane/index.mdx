...
- **Scope root**: `<state-dir>/retes/<scope>/` (user mode `$XDG_STATE_HOME/flor`, system mode `/var/lib/flor` — see [Agent § Node layout](agent#node-layout) and [Installer § Directories](../installer#directories)) — the identity store (`ca.crt`, `certs/`), immutable artifact sets under `artifacts/`, and durable state split by owner: `agent/` for the supervisor, `workloads/<name>/` per supervised workload.
