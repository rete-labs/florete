...
- **Source of truth is a git repo of YAML manifests, hand-edited.** No embedded DB. No CLI mutators that round-trip through YAML. Git diffs are the audit log; YAML comments explain intent. The source bundle uses Kubernetes-like Florete resources (`apiVersion`, `kind`, `metadata`, `spec`) but does not depend on Kubernetes.
...
- **Principals are uniform.** Users, services, and nodes are principals with cluster-scoped identities. Roles attach to any principal. Users and services own workload identities (end-to-end mTLS); each node has a cluster-member identity used by its `flor-agent` workload to reach the cluster's own infrastructure services (config-server, metrics). Each principal's cert+key are *held* by a flor vertex acting on the principal's behalf — the agent included. (C1 adds cluster-vertices as a further, distinct principal kind for hop-by-hop mesh transit.)
- **Access is fully derived in C0.** With no multi-hop routing, who-reaches-what is determined by `FloretePrincipal`, `FloreteService`, `FloreteRole`, and `FloreteGroup` resources plus service location. No `paths.yaml`, no label allocation — the compiler walks the ACL matrix directly.
...
- **Mgmt artifact is signed by the operator; agents verify locally.** The signing key is a SPIFFE principal in its own right — `management-plane/<name>` (convention: `primary`), a sign-only identity issued explicitly via `florctl ca sign --kind management-plane` and unrelated to any `FloretePrincipal` resource (see [ADR-0005](../../../contributing/adr/0005-use-spiffe-identities)). The operator holds three keys total: CA root, their own `user/<op>` TLS keypair (issued like any other user), and the cluster's `management-plane/<name>` envelope-signing keypair. None of them leave operator hardware — the cluster's coordination server (today the config-server, B1+ a managed cloud service) only stores and serves; it cannot mint or modify state. C0 is the degenerate case: every artifact is signed mgmt and there are no control-plane decisions yet. The envelope carries a `plane: "mgmt"` discriminator and a `signature` field from day one so a parallel `plane: "ctrl"` stream (signed by a `control-plane/<name>` principal) can land in C1 without schema churn. See [Decision authority layers](#decision-authority-layers) below for the evolution target.
...
A computing host may join more than one Florete cluster — for example, a developer's laptop enrolled in both a personal cluster and a work cluster, or a server node managed by two independent operators. This is a first-class use case and the design accommodates it without any special mode.

### Cluster scope

Each cluster a node joins gets its own **cluster scope**: a per-cluster namespace for runtime state and installed material on that host. The scope is identified locally by the cluster name (from `FloreteCluster.metadata.name`). The cryptographic anchor is the bundle's CA cert and operator pubkey — two clusters with the same name can never be confused because their trust roots differ. The local name can be overridden at install time (`flor enroll <bundle> --as <name>`) if two independent clusters happen to share one.

Per cluster scope on the node:

- **Install root**: `~/.flor/clusters/<scope>/` — CA cert, all principal certs and keys for this cluster, compiled artifacts (`agent.json`, `vertices/*.json`), agent control socket (`agent.sock`).
- **Runtime root**: `/run/flor/<scope>/` — FlorIO sockets and any other runtime files (C1+).
- **Process tree**: one `flor agent` process supervising its per-cluster vertex graph.
...
### Node identity in multiple clusters

A computing host that joins N clusters has N separate `node/<name>` principals — one in each cluster's trust domain. The names may differ across clusters (each cluster's operator chooses them independently). Each principal is acted on by a dedicated flor-agent process and held by that cluster's flor vertex. There is no shared node identity across clusters; the trust domains are fully independent.

### Local listen addresses

Two categories of listen addresses arise in a multi-cluster setup, and they have different audiences and different answers.

**External UDP listeners** appear only on nodes that accept inbound connections — nodes with a fixed public or private `IP:port` declared in their `FloreteNode` manifest. User devices (laptops, phones) sit behind NAT and use ephemeral source ports; they have no fixed UDP listen address. Therefore:

- External UDP conflicts can only arise on server nodes, which are by definition admin-managed (datacenter, office LAN, home server). Two cluster operators wanting the same external UDP port on the same physical server must coordinate with the IT admin responsible for that host. At C0/C1 scale this involves a small number of ports and technically capable people.
...
- Apps that need the actual port query the agent control socket — already the authoritative source for per-cluster runtime state.
- An ergonomic default: each cluster scope is assigned a **local-port offset** at install time, deterministic from the cluster CA fingerprint (or operator-chosen at `flor enroll` time), so cluster A's SOCKS5 lands at `:1080`, cluster B's at `:1180`, and so on. Single-cluster users get their expected default; multi-cluster users get predictable separation without any manual configuration.
...
- [ ] YAML schemas for all source resources; `FloreteCluster` carries per-category crypto (`ca`, `signers.mgmt`, `tlsPrincipals`); repo cert layout `certs/ca.crt` + `certs/management-planes/<name>.crt`; `enrollment.log` format
- [ ] Compiled-artifact schemas: `agent.json` (`artifactClass: agent`) and `flor.json` vertex config (`artifactClass: vertex`), semver'd, envelope with version stamp + `authorized_mgmt_signers` snapshot
...
- [ ] Reserved-name handling: `node` / `operator` roles and `config-read` / `config-write` groups must be present as source manifests (template); validator enforces; compiler auto-assigns `node` role to every node principal; operators are assigned manually via `spec.roles: [operator]` in `FloretePrincipal` resources; the cluster's `management-plane/<name>` signer is *not* auto-derived — it's issued explicitly via `florctl ca sign --kind management-plane --name <name>` as part of operator bootstrap (convention: `name = primary`)
...
- **Per-cluster local-port offset scheme** — deterministic port offset derived from cluster CA fingerprint (or operator-chosen at `flor enroll` time) so multi-cluster installs get predictable port separation without manual config (e.g. cluster A at `:1080`, cluster B at `:1180`). Ergonomic nicety; single-cluster users always see the default.
- **Port-query on agent control socket** — extend `flor agent status` (or a dedicated subcommand) so apps can ask "what's the current SOCKS5 port for principal X?" instead of hard-coding the preferred port. Required for apps that must work correctly on multi-cluster hosts; optional for single-cluster users where the preferred port always wins.
- **B1+ node-side capability advertisement** — node declares which external `IP:port` pairs it has available; cluster mgmt-plane consults that at compile time rather than having the cluster operator guess external UDP ports. Eliminates the remaining admin-coordination requirement for server nodes in multi-cluster setups.
