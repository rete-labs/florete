...
- **Install root**: `<root>/retes/<scope>/` (user mode `~/.flor`, system mode `/var/lib/flor` — see [Agent § Node layout](agent#node-layout)) — the identity store (`ca.crt`, `certs/`), immutable artifact sets under `artifacts/`, durable state under `state/` — per owner: `state/agent/` for the supervisor, `state/<name>/` per supervised workload.
- **Runtime root**: `/run/flor/<scope>/` (or the user-mode equivalent) — the agent control socket (`agent.sock`), FlorIO sockets (C1+), and other runtime files.
