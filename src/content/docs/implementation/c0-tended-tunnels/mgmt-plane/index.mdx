...
- **Process tree**: one `flor agent` process supervising its per-cluster vertex stack.
...
Compiled artifacts declare listen addresses for SOCKS5 inbounds, upstream TCP endpoints, and (in C1) UDP connection-manager listeners. These are provided by the cluster operator as **defaults**, not guarantees. The node operator is responsible for resolving conflicts:

- **SOCKS5 and per-service inbound ports** — cluster operators pick defaults; if two clusters' defaults collide on the same host, the node operator adjusts one at install time via a local port-override config. Bind failures surface at agent start and are logged clearly.
- **External UDP addresses** (`nodes.yaml` in C1) — the cluster operator declares which external IP and port this node is reachable on for that cluster's mesh. If the same host joins two clusters with overlapping external-address declarations, the node operator must ensure the UDP ports are distinct — they own both bundles and are the natural coordinator.

This is a deliberate split of responsibility: cluster operators control cluster-wide policy; the node operator controls node-local resources. Neither can unilaterally dictate the other's domain.
