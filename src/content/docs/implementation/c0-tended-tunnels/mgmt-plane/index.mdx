...
A computing host that joins N clusters has N separate `nodes/<name>` principals — one in each cluster's trust domain. The names may differ across clusters (each cluster's operator chooses them independently). Each principal is acted on by a dedicated flor-agent process and held by that cluster's flor vertex. There is no shared node identity across clusters; the trust domains are fully independent.
