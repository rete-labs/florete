...
- **Scope root**: `<state-dir>/retes/<scope>/` (user mode `$XDG_STATE_HOME/flor`, system mode `/var/lib/flor` — see [Agent § Node layout](agent#node-layout) and [Installer § Directories](../installer#directories)) — the identity store under `identity/` (`ca.crt`, `rete.json`, `certs/`), immutable artifact sets under `artifacts/`, and durable state split by owner: `agent/` for the supervisor, `workloads/<name>/` per supervised workload.
...
- **Flow B / CSR-based enrollment (open question for C1).** A security-purist path where the user generates its own keypairs and sends only CSRs, so private keys never leave its machine. Design-shaped questions: the CSR-bundle format, how the operator maps declared principal names onto an incoming CSR set, and whether the onboarding-UX cost is worth it while the operator is the Florete team. C0 enrolls via Flow A only (operator-generated keypairs); the `flor id create` primitive exists but isn't wired in. See [Enrollment](enrollment#per-node-enrollment).
- **Encrypted bundle & one-time-URL delivery (open question for C1).** In C0 the Flow A bundle is a plain file (bearer material — it carries private keys) delivered over any out-of-band channel. Hardening this into an encrypted, short-lived, personalized-URL delivery raises its own design questions (encryption scheme, key hand-off, URL hosting and expiry) and is deferred.
- **Operator-side keypair/CSR minting for enrollment (open design; C0 design ticket).** An unresolved seam in the enrollment CLI: `retectl issue-bundle` (Flow A) generates principal keypairs on the operator's machine, and `retectl ca sign --kind management-plane` reads as if it *mints* the mgmt-signer keypair — but the [CLI surface](cli) documents `retectl ca sign` as consuming an existing CSR (sign-only). Decide between a first-class `retectl` mint/issue primitive (shared by `issue-bundle` and the mgmt-signer step) and a shared keypair+CSR library used by both `retectl` and `flor id create` (leaving `flor id create` for Flow B only). Script-wrapping the `flor id create` CLI from `retectl` is explicitly out.
