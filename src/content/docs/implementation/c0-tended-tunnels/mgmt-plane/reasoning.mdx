...
Enrollment-event audit is preserved without committing certs: in C0 each sign action is a git commit that adds the principal (operator, principal, role, and timestamp in the commit metadata, plus the diff). C1 adds a dedicated append-only `enrollment.log` (sign/revoke record with cert fingerprints) — a strictly better audit than cert-in-repo, because it records the *decision* rather than just the resulting artifact.
