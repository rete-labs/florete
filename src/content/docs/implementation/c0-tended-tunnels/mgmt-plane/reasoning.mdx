...
Replacement: nodes fetch compiled artifacts from a **Florete-protected coordinator** run on a management node — same mTLS auth path as every other service, same SPIFFE identity model, no new credential kind. See [Why a coordinator, not direct git pull by nodes](#why-a-coordinator-not-direct-git-pull-by-nodes) below for the detail.

The git repo stays — as the operator's *authoring* workspace and shipment audit log. It holds YAML and the committed compiled tree — no certificate and no key; its commit history is C0's revocation audit trail — it records a principal's declaration and removal, never an issuance. Nodes never see it. (C1 adds a dedicated append-only `enrollment.log`; see [C1 Scope](/docs/implementation/c1-manual-mesh/scope).)
...
7. **Committed artifacts = inspectable, auditable shipments.** "What config did alpha actually get?" = `git log .flor/compiled/alpha/`. No need to reconstruct from YAML. Artifacts contain only path references (no secrets), so committing them is safe. The coordinator is the live wire path; git is the audit trail.

This is the same pattern as nginx.conf → loaded state, Terraform HCL → plan.json, or `kubectl apply` → etcd. In C0 the per-node output is two artifacts (`agent.json` + `flor.json`); in C1 a third is added for the second flor vertex in the recursive stack. The split is between supervisor (agent) and data-plane (vertex), so each consumer's contract stays minimal.
...
The root CA's private key stays on operator hardware, in `retectl`'s own tree, and never in the repo. **C0 protects it with filesystem permissions and nothing else** — passphrase protection lands in C1, and hardware custody (a PIV token, an HSM) in B1+ alongside the intermediate that makes an offline root practical; see [Enrollment § Decision Log](../enrollment#decision-log), ENRL-C0-06. Revocation via ACL removal is sufficient at C0 scale; CRL/OCSP is a B1+ concern.

**Why we don't pin cert fingerprints in `allow` entries.** A tempting "hardening" is to list each authorized cert's fingerprint next to its SPIFFE ID, to reject a rogue-CA cert bearing the same ID. It buys nothing in the rete model: a rete trusts exactly one CA (its own root), so a cert signed by any other CA fails the mTLS handshake outright, whatever SPIFFE ID it claims — there is no "same-ID cert from a different CA" to defend against. Fingerprint pinning earns its keep only in a multi-CA PKI, where you can't predict which of many trusted CAs signs a given identity; that is not the rete model, nor even the small-federation model. The genuine federation concern — ensuring a peer's SPIFFE **trust domain** is validated against the *right* CA rather than merely some trusted CA — is a different mechanism, tracked in [flor#51](https://github.com/rete-labs/flor/issues/51).

## Why no certificate lives in the repo

Signed identity certs are **public material** (public key + name + CA signature; nothing secret) — but they aren't *semantically required* in the rete repo. Each principal holds its own cert locally and presents it at mTLS handshake; verifiers only need the rete's trust anchors and the expected identity name.
...
- **No certificate in the repo; anchors and certs delivered via the enrollment bundle** — clean separation. The repo holds YAML source and the compiled tree, which is what compile and audit need; every certificate lives in `retectl`'s tree or on the node.

What the repo audits without committing certs is the **decision**: each git commit declares or removes a principal (operator, principal, role, and timestamp in the commit metadata, plus the diff). It does **not** audit issuance — `retectl issue-bundle` mints keys and signs certificates while committing nothing, a declaration may precede issuance by any interval, and re-issuance leaves no trace ([Enrollment § Decision Log](../enrollment#decision-log), ENRL-C0-24). Committing certificates would not have closed that gap either; it would record the resulting artifact and still miss the act. C1's dedicated append-only `enrollment.log` (sign/revoke record with cert fingerprints) is what supplies the issuance half.
...
## Why C0 has no inter-node links (and no mesh-vertex identity)
...
This means C0 has **no mesh-vertex identity layer**, no `links.yaml`, no `paths.yaml` — the hop-by-hop mesh crypto layer simply doesn't exist. Workload access is fully derivable from users + services + roles + groups without any routing configuration. The agent at each node carries its own principal identity (for mgmt-plane communication with the coordinator), but that's orthogonal to the workload data plane and to mesh transit.
...
Replacement: **operator issues a bundle per node** (covering the node's identity and every workload it hosts); delivery is out-of-band (Telegram, email, any channel); user runs `flor enroll`. The bundle contains the signed certs, the trust anchors and the node's initial artifact set — no coordinator URL (the artifact set already names the coordinator), no git credential, no deploy key.

This is how WireGuard / OpenVPN enterprise deployments work. Users never see git; operator is the only writer. The model also accommodates a Flow B for security-conscious users (principal generates keypairs, sends CSRs out-of-band, operator returns signed certs) that preserves key-hygiene — though C0 ships Flow A only; Flow B is deferred (an open question for [C1](/docs/implementation/c1-manual-mesh/scope)), see [CLI Surface](../cli).

## Why a coordinator, not direct git pull by nodes
...
The third option — the coordinator — wins on every axis that matters at our scale: fewer moving parts, one access-control system, no forge lock-in, no extra credential to rotate. The cost is that we now have one always-on server per rete (the management node), but that node is the rete's designated infrastructure host regardless — it is where the coordinator (and, C1+/B1, the metrics endpoint) lives, just another Florete-published service.
...
**Management-node bootstrap is the only inherent drawback.** The coordinator can't serve its own artifact before it's running. The one-shot manual bootstrap (scp the bundle + `mgmt01.json`, start the agent) is documented as a playbook step and handles this cleanly — once `mgmt01` is up, everything else flows through it.

## Why C0 defers commit-timeout (and leans on out-of-band access)

Publishing SSH over Florete means a bad `flor agent sync` can remotely brick a server. This is the classic network-device failure mode, and the industry-standard mitigation is `commit confirmed`: activate the change, require explicit confirmation within a timeout, else auto-rollback (Cisco IOS, Juniper JunOS, and most serious network-equipment CLIs default to it).

Florete will adopt exactly this pattern — but in **[C1](/docs/implementation/c1-manual-mesh/scope), not C0**. For C0's limited pilots the cheaper mitigation already in the design is enough: keep a non-Florete emergency-access path (cloud console, hypervisor, admin VPN, or an unpublished backup SSH port — see [Safety Net](../distribution-and-reload#safety-net-server-nodes)). Pilots never rely on Florete as the single way in, so a bad rollout is recoverable without an auto-rollback guard. Deferring it keeps C0's sync path a plain fetch-install-restart and moves the confirm/rollback state machine — which must also survive an agent crash mid-window — to the milestone where it's designed properly.

The `commit confirmed` pattern is still the right end state: it eliminates the highest-severity failure mode and trains operators into a safe habit from day one. That's why C1 adopts it rather than something bespoke.
...
|--------------------------|------------------------------|----------------------------------|
| C0 Tended Tunnels        | `retectl compile` (local)    | bundle + coordinator fetch     |
| C1 Manual Mesh           | `retectl compile` (local)    | bundle + coordinator fetch     |
...
The rete's coordination point — coordinator today, a managed cloud service in B1+ — is the obvious blast-radius target. If it's compromised and it can mint or modify access state, an attacker who hijacks it owns the rete. The defense is to keep the signing key off the coordination point entirely: the operator signs every mgmt artifact on their own hardware with a dedicated `management-plane/<name>` signing principal (a sign-only identity issued separately from any user, distinct from the operator's `user/<op>` TLS principal — see [ADR-0005](/docs/implementation/adr/0005-use-spiffe-identities)); agents verify locally against the certificate the artifact itself carries, validated against the trust anchors delivered at enrollment. The coordination point only stores and serves. It cannot grant access, change topology, or replace identities — only refuse to serve.
...
1. Sign every mgmt artifact. Single `management-plane/<name>` signing principal (issued separately from any user; `mgmt-signer` by convention), file-backed, ed25519. Its cert is sign-only (no TLS `extKeyUsage`) so a leak can't be re-used to impersonate any operator's or workload's TLS identity. Agents verify on every fetch.
...
- Node fast-path autonomy primitives (FRR-class). C0/C1 have nothing dynamic to react to.
...
Identity issuance in C0/C1 is **operator-driven and disconnected**: `retectl issue-bundle` runs on the operator's machine, mints certs from the root CA held in `retectl`'s tree, and delivers them out-of-band. *Disconnected* rather than *offline* — the root key is read on every issuance, which is exactly why C0 does not pretend it is offline and why a genuinely offline root waits for the intermediate in B1+. Validity is 90 days; renewal = the operator runs the same command before expiry. There's no automation because automation needs an online signer, and the whole zero-trust-cloud posture rests on "no online signing key."
...
What we do **now** in C0/C1: nothing on rotation automation. The operator-signed mgmt artifact references principals by SPIFFE ID only (identity material lives in the node's identity store), so future rotation replaces store files in place without recompiling the artifact — the schema doesn't need changing.
