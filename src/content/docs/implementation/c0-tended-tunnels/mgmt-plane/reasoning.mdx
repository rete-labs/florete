...
The git repo stays — as the operator's *authoring* workspace and shipment audit log. It holds YAML, `ca.crt`, and the committed compiled tree; its commit history is C0's enrollment/revocation audit trail. Nodes never see it. (C1 adds a dedicated append-only `enrollment.log`; see [C1 Scope](/docs/implementation/c1-manual-mesh/scope).)
...
Replacement: **operator issues a bundle per node** (covering the node's identity and every workload it hosts); delivery is out-of-band (Telegram, email, any channel); user runs `flor enroll`. Bundle contains the signed certs, CA cert, coordinator URL, and expected coordinator SPIFFE ID — no git credential, no deploy key.
