...
1. **Unpack** — create `retes/` and the scope root beneath it, then write the identity store and the initial artifact set into it. Needs the bundle; happens once per scope per node. The installer creates neither: `retes/` is the agent's tree, and enroll is what brings a scope into existence ([Installer § What installing is](../../installer#what-installing-is)).
2. **Activate** — register and start that scope's service wrapper. Needs no bundle, no operator, and no network.

`flor enroll <bundle>` performs both. Uninstall reverses only the second, so restoring service is `flor install` plus an activation — which is why step 2 gets a name of its own instead of a bundle-less `enroll` that enrolls nothing: **`flor agent enable [--rete <scope>]`**, with `disable` as its inverse, mirroring `systemctl enable` because it is largely what it wraps — and auto-detecting the scope when only one rete is enrolled, exactly as `status` and `sync` do, since the single-rete case is the common one. `flor install` lists surviving scopes and prints the command for each rather than starting anything itself, so install's starts-no-service rule survives intact.
