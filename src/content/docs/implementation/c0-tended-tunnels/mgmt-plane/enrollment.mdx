...
2. Operator: `retectl issue-bundle --node alice-laptop --rete <coordinator-url> --validity 30d --out alice-laptop.bundle`.
   ...
   - Packages: `ca.crt`, node cert + key, per-principal cert + key, coordinator URL, expected coordinator SPIFFE ID, plus a current `agent.json` and `vertices/flor.json` for alice-laptop — into a single bundle **file** (`alice-laptop.bundle`).
...
5. Alice installs `flor` (per the docs install guide) and runs `flor enroll alice-laptop.bundle`, which writes `~/.flor/retes/rete-lovers/{ca.crt, alice-laptop.crt, alice-laptop.key, alice.crt, alice.key}` plus the bundled `agent.json` and `vertices/flor.json` into that rete scope, then starts `flor agent run`. The agent brings up the vertex and immediately runs `flor agent sync` once to pick up any newer state from the coordinator.
