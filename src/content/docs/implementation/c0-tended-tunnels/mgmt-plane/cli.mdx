...
- `flor agent sync` fetches `<this-node>/{agent.json, vertices/flor.json}` from the rete's coordinator over Florete (acting as the `node/<node>` principal via the local vertex's SOCKS5), installs the new artifacts, and restarts the supervised vertex with a commit-timeout guard. No compilation on the node — artifacts were pre-compiled by the operator and published to the coordinator.
