...
```json
{
  "artifacts": [
    ...
    { "schema_version": "1.0", "plane": "mgmt", "version": 42, "node": "alpha", "name": "hop",
      "generated_at": "…", "payload": { … }, "signature": { … } }
  ]
}
```
...
The coordinator's durable state lives under its spawn-injected `FLORWL_STATE_DIR` (`workloads/coordinator/` in the scope root — [Agent § Node layout](../agent#node-layout)); it derives no paths from names and receives none in config:
...
```json
{
  ...
  "signature": {
    "cert": "<base64-der-x509 of the mgmt signer>",
    "value": "<base64>"
  }
}
```
...
**Two principals, one workload.** The artifact and supervision-inventory `name` is `coordinator`, naming the workload `workload/<node>/coordinator`. That it matches the service principal `service/coordinator` means nothing: names of different kinds may coincide, and the coordinator's workload owns both service principals ([Identity](../identity#decision-log), ID-C0-11, ID-C0-03). `coordinator-publisher` exists only as a principal in the vertex payload: no artifact, no supervision entry. The one process binds both loopback addresses.
...
**Management-node bootstrap (chicken-and-egg).** The coordinator can't fetch its own artifacts from itself before it's running. Resolution: the management node is bootstrapped manually, once. The operator compiles and commits, then runs `retectl issue-bundle --node mgmt01`; the resulting bundle carries that node's complete initial set — `agent.json`, the vertex's `hop.json`, **and `coordinator.json`** ([Enrollment](../enrollment)); `flor enroll` writes it and `flor agent enable` starts the agent, which spawns the vertex and the coordinator like any supervised workloads. Once the two services are reachable over Florete, the operator runs `retectl publish` — the first Florete-over-Florete call — and from then on `mgmt01` refreshes itself via the normal `flor agent sync` flow, coordinator config included. The manual step is genuinely one-shot per management node — and it needs no exemption to run, because issuing a bundle requires only a committed compiled tree and never a reachable coordinator ([Enrollment § Decision Log](../enrollment#decision-log), ENRL-C0-35). That is what keeps this path available for a **rebuild** as well as for a first boot: a management node lost to hardware failure is replaced by exactly this sequence, which would be impossible if cutting its bundle required publishing to the coordinator it is replacing.
