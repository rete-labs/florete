...
A scope that survives an uninstall is **enrolled but inactive**, not "ready to enroll": reinstalling and running `flor agent enable` resumes it with no bundle and no operator involvement. That property belongs to enrollment's own two-step shape rather than to this page — see [Enrollment § Activation and resume](../mgmt-plane/enrollment#activation-and-resume).
...
- **Embedded image builds (B2-era).** The same call against a staged rootfs; Yocto-class recipes wrap it. Pairs with [enrollment's](../mgmt-plane/enrollment#per-node-enrollment) bake-the-bundle-into-the-image provisioning.
