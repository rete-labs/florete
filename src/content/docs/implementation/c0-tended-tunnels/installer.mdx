...
```log
  ...
  receipt     install-receipt.json: phase, schema version, release version, the
              ...
              what it lists is what the installer owns; the version it names as
...
```log
  ...
  & upgrade   * marks a step only an upgrade takes
              ...
              *  8  re-render every per-scope wrapper copy
                 9  replace the receipt, phase installed
              * 10  restart exactly what step 6 stopped
                11  report the mode, the directories, the version, any PATH
...
```log
  ownership     the channel owns the host — files, wrappers, service lifecycle,
                ...
                the installer never touches retes/
...
**The channel owns the host; the agent owns the rete.** Placing files, updating the service wrappers, and stopping and starting the services around a swap are *host* business, and every supported channel does them — `install.sh` here, maintainer scripts and post-install hooks elsewhere. Artifacts, node state, migrations and identity are *rete* business and belong to the agent, which is why **the installer never touches `retes/`**. The line falls where the knowledge does: wrappers are the only OS-specific artifacts in the system, so the component deliberately kept OS-agnostic must not be the one rendering them, while node state outlives every installation of every channel and so cannot belong to whichever one happens to be installed.
...
- **writes an install receipt** — `install-receipt.json` in the state directory: a `phase`, a receipt `schema_version`, the release version, **the release it replaced**, and the exact list of files placed. The previous release is what makes going back possible at all, since nothing else on the host names the version that worked ([Downgrade](#downgrade)). It records neither the mode — a property of *where the receipt is*, each mode having its own state directory — nor an origin, which would be tautological in a file only this channel writes;
...
| **State** | `retes/<scope>/` scope roots, the receipt, the lock, the marker | `$XDG_STATE_HOME/flor/` — `~/.local/state/flor/` unset | `/var/lib/flor/` |
...
3. **Do not restart it on exit `78`.** `78` (`EX_CONFIG`) means *this config is not servable* — a rejected artifact, or node state this release cannot read. Looping on it converts a clear stop into noise. Where a service manager cannot express the exemption, the fallback is the agent's own refusal plus that manager's restart throttling: the refusal still does its work, the logs are just louder.
4. **Own the per-scope directories it can** — create the runtime root at start, remove it at stop, so no stale socket outlives an instance.
...
| Exempt `78` | `RestartPreventExitStatus=78` | no equivalent — throttling plus the agent's refusal | recovery policy |
| Per-scope dirs | `StateDirectory=flor/retes/%i` + `StateDirectoryMode=0700`, `RuntimeDirectory=flor/%i` + `RuntimeDirectoryMode=0750` | agent creates them | agent creates them |
| Stderr goes to | the syslog socket, [always](#logging) | the syslog socket, [always](#logging) | Event Log |

Three mappings deserve their reasoning stated rather than read off the table.

**systemd's directory directives are per-instance, and that is the point.** An instance is one rete scope, so it owns its scope root (`<state-dir>/retes/<scope>/`) and its runtime root, and nothing above them. The shared parent — flor's state directory itself, holding the installer's own bookkeeping and the `retes/` directory every scope lives under — is **installer-owned, not systemd-owned**: it outlives every instance and belongs to no one rete, which is the [no-node-global-state invariant](../mgmt-plane/agent#role) written as directives. The pair differ only in lifetime, as their names say: systemd removes the `RuntimeDirectory` when an instance stops and leaves the `StateDirectory` in place. Units land in `/etc/systemd/system`, systemd's location for admin-placed units, as opposed to `/usr/lib/systemd/system` where a distro package would put its own.

**Permissions are stated, never defaulted — and ownership is enrollment's to establish, not systemd's to repair.** `StateDirectoryMode=` defaults to `0755`, which would leave `/var/lib/flor/retes/<scope>/` — the directory holding the [identity subtree](../mgmt-plane/agent#node-layout) — world-traversable, so the design names `0700` explicitly. `RuntimeDirectoryMode=0750` is the one deliberate loosening: `agent.sock` must be reachable by an operator CLI without root ([CLI § State directory](../mgmt-plane/cli)), and membership in the `flor` group is that grant — worth being an explicit group rather than a slack mode bit, because it permits reading node state and triggering a sync. But **no private key's mode may depend on a service manager starting later.** In system mode `flor enroll` runs as root, and it creates the scope root already owned by the service user and already restricted: `retes/<scope>/` `0700 flor:flor`, `identity/` `0700`, every `*.key` `0600` — each created *with* its final mode rather than chmod-ed afterwards (write into a mode-`0600` temp file in the target directory, then rename), because a key must not exist world-readable even for the instant between two syscalls. The directives then agree with what is already on disk instead of fixing it. Above the scope root nothing changes hands: flor's state directory and its `retes/` stay **root-owned `0755`** — the installer's, per the split one paragraph up — which is also what keeps a compromised agent from rewriting the receipt describing the installation it runs from. User mode keeps the same modes with the invoking user as owner, so a shared machine leaks nothing either.
...
**The file is created if absent and never overwritten.** It exists to be hand-edited, so an upgrade leaves an existing one alone and *reports* when the release's defaults have moved; the macOS probe runs only when the file is being created, not on every upgrade. A plain uninstall keeps it and `--purge` removes it — dpkg's conffile behaviour, both halves of it: the edit survives an upgrade and a removal, and only a purge takes it away.

An admin who prefers a different daemon edits the one line and restarts the agents. A host where nothing answers is misconfigured, and the agent says so at startup rather than discarding its own diagnostics.
...
2. **Write the receipt with `phase: installing`**, recording the release being replaced and which scopes are currently active. That record has to be durable rather than in memory: a crash between here and step 6 would otherwise lose the one fact a re-run cannot reconstruct, and the *no change to activation state* rule would fail in exactly the situation worth surviving.
...
4. **Rename-swap the binaries.** Per-path atomicity: a crash leaves each path old-or-new, never truncated, and no `ETXTBSY` if some process we did not stop still holds the old inode — a hand-started `flor vertex run` from [debugging](../mgmt-plane/cli), say.
...
Placing files, updating the service wrappers, and stopping and starting the services around a swap are **host** business, and every supported channel does them — `install.sh` here, maintainer scripts and post-install hooks elsewhere. Artifacts, node state, migrations and identity are **rete** business and belong to the agent, which is why the installer never touches `retes/`.
...
**INST-C0-18. Every mutating installer operation holds one advisory lock, and nothing reads its file.**
An `flock` on `<state-dir>/install.lock`, taken by install, upgrade, uninstall and `--purge` alike. **Nothing decides anything from the file's presence** — its only job is to be locked — so a plain uninstall leaves it where it is, and `--purge` removes it with the directory as its last act.
...
*Rejected* the receipt as the lock target — `flock` binds to an inode and the receipt is replaced by rename, so the holder's lock would protect nothing; locking the state directory itself — Windows byte-range locking is defined over file handles rather than directory handles, and an invisible lock is undiagnosable where a named file answers "why is my install waiting" with `lsof`; deleting the lock on a plain uninstall for tidiness — it buys a cleaner listing at the price of the purge race, in the one case that does not have to run it.
...
**INST-C0-23. `config.toml` is created if absent and never overwritten.**
An upgrade leaves an existing file alone and *reports* when the release's defaults have moved. `--purge` removes it; a plain uninstall does not. The macOS syslog-socket probe runs only when the file is being created.
*Because* the file exists to be hand-edited, and dpkg's conffile behaviour is the settled shape of this problem — both halves of it: an upgrade that keeps your edit, and a `remove` that keeps it too, with only `purge` taking it away.
*Accepting* that a host whose syslog daemon changes after install keeps a stale destination until someone edits the line; the report is what surfaces it.
*Rejected* replacing it on upgrade — destroys the edit the file exists for; leaving it unowned — a file no rule lets even `--purge` remove.

**INST-C0-24. flor logs to a syslog socket on every supported platform, and the installer ships no log file machinery.**
The installer writes the one setting and nothing else: `unix:/dev/log` on Linux; on macOS it probes the paths a real syslog daemon conventionally listens on and writes the first that accepts a datagram.
...
*Because* before enrollment there is nothing to run; and because `78` means *this config is not servable*, so looping on it converts a clear stop into noise and, mid-upgrade, into repeated attempts to run a half-swapped set.
...
**INST-C0-27. Permissions are stated, never defaulted, and no key's mode may depend on a service manager starting later.**
Scope roots `0700`, `certs/` `0700`, every `*.key` `0600`, each created *with* its final mode rather than chmod-ed afterwards. The runtime root is `0750` so an operator CLI can reach `agent.sock` through `flor` group membership. Above the scope root, flor's state directory stays root-owned `0755`.
*Because* `StateDirectoryMode=` defaults to `0755`, which would leave the directory holding private keys world-traversable — and a key must not exist world-readable even for the instant between two syscalls. Keeping the parent installer-owned is also what stops a compromised agent from rewriting the receipt describing the installation it runs from.
...
Take the lock; write the receipt with `phase: installing`, recording the release being replaced; stop the active agents; stage and rename in the executables and the wrapper template, re-rendering every per-scope copy; replace the receipt with `phase: installed`; restart exactly what was stopped; report. Everything before the rename-in is a free abort: the phase is one field, and clearing it undoes the run completely.
...
*Rejected* migrating as part of this sequence — INST-C0-03 moved that to the agent, which removes the ordering problem rather than solving it; a not-runnable marker the agent must read — it would work on this channel and on no other, which is exactly the defect INST-C0-03 exists to prevent.
...
Plain uninstall deactivates every instance and removes what it placed: the wrappers (template and every rendered copy), the runtime roots, the binaries, the receipt and the lock. Three things stay, each for its own reason — the **scope roots**, which were never the installer's; **`config.toml`**, which is admin-owned once created (INST-C0-22); and the **`flor` user and group**, whose uid owns those surviving scope roots. `--purge` removes the state, config and runtime directories entirely, private keys included.
