...
A scope that survives an uninstall is **enrolled but inactive**, not "ready to enroll": reinstalling and running `flor agent enable` resumes it with no bundle and no operator involvement. That property belongs to enrollment's own two-step shape rather than to this page — see [Enrollment § Activation](../mgmt-plane/enrollment#activation).
...
**INST-C0-26. The installer ships the wrapper machinery; enrollment activates it per scope.**
