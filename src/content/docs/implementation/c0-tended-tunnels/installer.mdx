...
Settled boundary, restated from [Agent § Supervision mechanics](../mgmt-plane/agent#supervision-mechanics): **the installer ships the machinery, enrollment activates it per scope.** The installer places one wrapper per host — a template, instantiated per scope — and activates nothing; [enrollment](../mgmt-plane/enrollment) or `flor agent enable` does that.
