...
- A pilot is bounded by the credential lifetime, 90 days by default. C0 has no automated rotation: re-keying is a fresh bundle couriered per node ([Identity](/docs/implementation/c0-tended-tunnels/mgmt-plane/identity#decision-log), ID-C0-22), so a pilot outliving its credentials costs an operator visit to every node
