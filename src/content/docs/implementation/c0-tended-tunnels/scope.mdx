...
- Nodes are desktops, laptops and servers — machines with a maintained clock, an operator who can reach them, and no long disconnection. Embedded and vehicle-borne nodes are B2's, and nothing in C0 is sized for them
- A pilot is bounded by the credential lifetime, 90 days by default. C0 has no automated rotation: re-keying is a fresh bundle couriered per node ([Enrollment](/docs/implementation/c0-tended-tunnels/mgmt-plane/enrollment#decision-log), ENRL-C0-25), so a pilot outliving its credentials costs an operator visit to every node
...
**C0 observability is node-local.** Every flor process logs to the host's **syslog socket** — `/dev/log` on Linux, a real syslog daemon on macOS ([Installer § Logging](/docs/implementation/c0-tended-tunnels/installer#logging)); the user guide documents how to view and export those logs, and `flor agent status` gives a local state readout. Nothing is shipped to an external collection server in C0 — that upload path arrives with B1's coordinator. The full design — log conventions, metrics-ready counters, and their readout — is [Observability](../observability).
...
- **`retectl`** — the operator's authoring CLI (`init`, identity issuance, validate, compile, `issue-bundle`, `publish`). See [CLI Surface](/docs/implementation/c0-tended-tunnels/mgmt-plane/cli).
- **`flor`** — the per-node CLI and supervisor (agent), which owns the node principal, launches and restarts the vertex and coordinator (on the management node). See [Management Plane](/docs/implementation/c0-tended-tunnels/mgmt-plane) and [CLI Surface](/docs/implementation/c0-tended-tunnels/mgmt-plane/cli).
...
- **`flor-coordinator`** — the store-and-serve management-node service. See [Coordinator](/docs/implementation/c0-tended-tunnels/mgmt-plane/coordinator).
- **Installer** — `install.sh` bootstrapping a `flor install` core: one channel, GitHub Releases, two targets (`linux-x86_64`, `macos-arm64`; no Windows in C0 — see [Cross-platform](#cross-platform)). Design: [Installer](/docs/implementation/c0-tended-tunnels/installer).
...
For C0, our target OSes are **Linux and macOS** (macOS on Apple Silicon only — see [Installer § Platforms](/docs/implementation/c0-tended-tunnels/installer#platforms)). Windows will be added in [C1](/docs/implementation/c1-manual-mesh/scope) — the design stays portable (OS-agnostic core, thin OS-specific service wrappers), but a Windows deliverable is not part of C0. Later on (B1+) we'd like to support mobile OSes (Android, iOS, even HarmonyOS).
...
- **`enrollment.log`** (append-only sign/revoke audit) — introduced in [C1](/docs/implementation/c1-manual-mesh/scope). In C0 the git history of the rete repo records a principal's declaration and removal and nothing else; **issuance is unrecorded**, so C0 has a revocation record and no enrollment record ([Enrollment](/docs/implementation/c0-tended-tunnels/mgmt-plane/enrollment#decision-log), ENRL-C0-24).
- **Flow B / CSR-based enrollment** (principal-generated keypairs) — an open question for [C1](/docs/implementation/c1-manual-mesh/scope). C0 enrolls via Flow A only (operator-generated keypairs); Flow B's commands are named but unbuilt.
- **Encrypted bundles & fetched-credential delivery** — an open question for [C1](/docs/implementation/c1-manual-mesh/scope). In C0 the enrollment bundle is a plain file the operator delivers over any out-of-band channel; it is bearer material, and nothing at the bundle level bounds the window between issue and enroll.
...
- **Binary rollback in the installer** — retained previous binary sets and a `--rollback` that needs no network move to [C1](/docs/implementation/c1-manual-mesh/scope) with the sync guard above, for the same reason and as one story. C0 needs no guard of its own: any release installs over any other, so going back is installing the older one (see [Installer · Downgrade](/docs/implementation/c0-tended-tunnels/installer#downgrade)).
- **Node-state migrations** — the agent's and an operator-invoked command's, not the installer's; nothing in C0 has anything to migrate from (see [Installer · Migrations](/docs/implementation/c0-tended-tunnels/installer#migrations-are-not-the-installers)).
