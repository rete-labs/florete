...
**Two consequences found by implementing this, not by reasoning about it:**

- **Span level gates context.** An `info_span!` disappears under `RUST_LOG=warn`, taking the context of the WARN lines inside it with it — a regression against hand-interpolated context. Connection and operation spans are therefore `error_span!`: they exist whenever anything is logged at all.
- **`env_logger` was silently stripping ANSI.** Its buffer swallowed escape codes written into it; a `tracing` writer passes bytes through, so error-stack's own emphasis leaked into non-terminal output. The terminal check must therefore also set error-stack's color mode — the tty gate has two sources, and one of them was masked.
