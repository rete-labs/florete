...
Because neither the sink's framing nor "is this fd shared with my siblings" is observable from the fd, the agent states the class in `FLORWL_LOG_SINK`, and every process announces it at startup. This holds for a **provisioned** process. A process that opens its own sink — every flor process in a release with no supervisor ([DSGN-A0-16](/docs/implementation/a0-alef-authoring/design#decision-log)) — takes its class from what it opened, a socket being `syslog` and stderr `private`, and reads no variable that could contradict it. A process started by hand with neither has nobody to tell it and defaults to `private`; it probes for nothing, since a probe answers a different question than the one being asked. All three values may be set by hand, `private` included, so the default is sayable rather than only reachable by absence.
