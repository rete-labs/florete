...
**Harder — a new host dependency.** flor now requires something listening on a syslog socket, and refuses to start without it. On Linux that is free; on macOS it is a documented setup step ([user guide](https://github.com/rete-labs/florete/issues/14)); on a platform with no syslog socket at all it is [native-sink work](#why-macos-takes-the-same-class), never a file fallback. This is the price of deleting the file class, and it is paid at startup with a clear error rather than at 3am with a missing record.
