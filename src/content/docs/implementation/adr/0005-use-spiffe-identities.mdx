...
- Issuing a `control-plane` kind should be expressible in C0 even though no envelope signing happens yet, because the kind vocabulary is the user-visible contract and adding a value later is more disruptive than starting wide. *(Amended with C0 enrollment: the six kinds remain part of the identity model, but C0's CLI exposes a narrower slice — `retectl id issue` takes the signing kinds, and TLS-principal certs are issued inside `retectl issue-bundle` rather than one at a time. A CSR-consuming sign command belongs to the deferred Flow B.)*
