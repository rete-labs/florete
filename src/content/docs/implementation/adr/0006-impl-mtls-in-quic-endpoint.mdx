...
> **Amended by [ADR-0007](/docs/implementation/adr/0007-decouple-naming-identity-routing).** SNI is now an internal routing hint the server resolves by **registry lookup** (keyed by a kind-and-scope-marked label derived from the target, unrelated to the hostname), never by `dns::parse` — so the kind a vertex name would otherwise lose is preserved, and the wire label may later become opaque. The convenience-name *shape* below still holds for services as `ServiceHostname`'s `render` output, but it is only the *user-facing* name (resolved with context), and never the wire SNI; the trust domain may be dotted. The `dns::format` / `dns::parse` helpers named below are replaced by `render` / `resolve` on `ServiceHostname`, and `connect` takes a `Targetable`.
...
Internally, `connect` calls `Resolver::resolve(target) -> SocketAddr` to find where to dial, derives the SNI via `sni_for(target)` (see [ADR-0007](/docs/implementation/adr/0007-decouple-naming-identity-routing); `target` is a `Targetable`), builds a per-call rustls `ClientConfig` with the caller SVID and a `SpiffeServerVerifier` parameterised by the expected target, and dials.
