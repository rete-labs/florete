...
### Four versioned surfaces, each with its own ladder
...
3. **The enrollment bundle's manifest** — a `schema_version` claim in `manifest.json`, on its own ladder, gated by `flor enroll`. *(Added with the C0 enrollment design; see [ENRL-C0-16](/docs/implementation/c0-tended-tunnels/mgmt-plane/enrollment#decision-log).)* It is separate from the envelope contract because the two skew independently and for different reasons: an envelope is produced by the compiler and consumed by the agent on the **same release** wherever a node is current, while a bundle crosses from an operator's `retectl` to a node's `flor` **upgraded on unrelated schedules** — the one contract here where a newer producer meeting an older consumer is the ordinary case rather than the exotic one. The stamping rule below is what makes that survivable, and `retectl issue-bundle` obeys it: a bundle whose content uses no new construct keeps enrolling nodes nobody has upgraded.
4. **Opaque workload configs** — outside the flor contract entirely: schema and versioning belong to the producing/consuming pair. The coordinator's config exercises this path from C0 (both ends are flor's own, dogfooding the tenant contract); tenant configs follow.
...
- **Spawn-injected runtime context** — the `FLORWL_*` environment contract a supervisor passes at spawn: intra-node, same-install conventions rather than a document. It needs no schema version; anything that ever crosses a machine or authority boundary must instead graduate to an enveloped artifact rather than grow a second versioning regime.
