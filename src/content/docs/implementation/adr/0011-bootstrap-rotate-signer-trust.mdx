...
Obsoleted by [ADR-0015: Anchor Signer Trust in the Root CA](/docs/implementation/adr/0015-anchor-signer-trust-in-root-ca)

The pinned-set model recorded here has two defects that appear only in combination with decisions taken elsewhere: the chain has no anchor, so a leaked signer key is unrecoverable; and it requires seeing every rotation step, which a node absent across one never does, because the coordinator serves only the latest tree. ADR-0015 replaces it with the standard flow — the signer's certificate travels with what it signed and is validated against the node's trust anchors. What survives unchanged: enrollment as the out-of-band trust anchor, version monotonicity and the high-water-marks, and the agent as sole verifier.
...
### The agent is the sole signature verifier

The **agent** fetches every artifact and verifies envelopes and signatures — the authenticity of the producer; the **vertex** consumes agent-staged configs and, from C1, verifies the *semantic* mgmt↔ctrl cross-rules itself, being the only party that parses both planes (see the amended [ADR-0010](/docs/implementation/adr/0010-compiled-artifact-object-model) and the [Agent layering rule](/docs/implementation/c0-tended-tunnels/mgmt-plane/agent#supervision-contract)). This matches the threat model: an artifact signature protects the **distribution path** (coordinator, network MITM, a tampered relay), not local-filesystem integrity — anyone who can rewrite a staged `rete.json` on the node can equally rewrite the `flor` binary, so re-verifying inside the vertex guards nothing the signature is for. Local integrity is the node's own concern. (Vertex re-verification remains available as defence-in-depth; it is not the boundary.)
...
(The CA cert is not referenced here — it sits at its conventional place in the node's identity store.)
...
- **Rollback defence.** The envelope's monotonic `version` is the high-water-mark. For mgmt the agent keeps **a single mark for the node's set** — the set version, stamped uniformly across the set, so replaying any old set (e.g. an old `[key1, key2]` `agent.json` resurrecting a retired `key1`) is rejected wholesale; for ctrl it keeps **one mark per artifact stream**. The agent rejects anything below the held mark.
- **Persistence.** The agent durably persists `(trusted mgmt set, trusted ctrl set, version high-water-marks — mgmt set-level, ctrl per-stream)`. Without it, a restart re-pins from enrollment and silently loses rotation, reopening the replay window.
...
- The agent must maintain durable, integrity-protected local state (trusted sets + version high-water-marks); losing it degrades to the enrollment anchor and loses rotation.
