...
"links": [
  { "type": "enum", "members": [
    { "name": "beta-pub",  "peer": "spiffe://rete-lovers/vertex/beta/rete",  "via": { "adapter": "via-link" } },
    { "name": "beta-eth1", "peer": "spiffe://rete-lovers/vertex/beta/rete",  "via": { "adapter": "eth1", "udp_peer": "10.0.1.6:5544" } },
    { "name": "gamma",     "peer": "spiffe://rete-lovers/vertex/gamma/rete", "via": { "adapter": "internal", "udp_peer": "10.0.0.7:5544" } }
  ] }
]
```

Each link carries a local `name` (the forwarding handle — `beta-pub` and `beta-eth1` are two links to the *same* peer), its `peer` (the remote mesh-vertex identity, for link-layer mTLS), and exactly one `via` — `{ "adapter": <name>, …adapter-specific dial }`. A delegating adapter (FlorIO) carries no address (it hands dialing to the layer below); a wire adapter (UDP) carries `udp_peer` (it terminates the wire), so the recursion boundary stays legible: the address surfaces at whichever layer owns the wire. `via` lives **only** here; the compiler assigns stable link names, and an operator's `paths.yaml` hop `via:` (the source-side link selector) compiles down to a link `name`.

### `ingress`/`egress` are pure flat allow-lists

Both are `[{ "target_spiffe_id", "allow": [ <principal-spiffe-id>… ] }]` — operator authority, **not** bounds (the CP cannot narrow them, so no typed-rule shell). `egress` drops `target_node` (derivable from the peer's node-scoped SPIFFE ID), `target_udp` (moved to `links`), and `via`/`first_hop` (moved to ctrl). `egress` is now identical in shape to `ingress`.

### Routing realisation is ctrl; the mgmt↔ctrl join is a zip

mgmt contributes stable ACL-and-dial facts; ctrl contributes the routing realisation. **Labels are per-link, not per-peer or per-node** (MPLS per-interface semantics): a label is only meaningful in the label space of the link it travels on, and parallel links to the same peer have *distinct* spaces — so every label is paired with its **link name**. Each entry is an `in → out` mapping where each side is either a link-label (a transit hop) or a local workload (origination/delivery, named by `target`+`principal`):

```json
{
  "schema_version": "1.0", "plane": "ctrl", "kind": "vertex",
  "version": 17, "obeys_mgmt_version": 42, "node": "alpha", "vertex": "rete",
  "payload": {
    "active_links": [ "beta-pub", "gamma" ],
    "label_bindings": [
      { "scope": "ingress", "target_spiffe_id": "spiffe://rete-lovers/service/api",
        "principal": "spiffe://rete-lovers/user/alice",
        "in":  { "link": "beta-pub", "label": 9 } },
      { "scope": "egress",  "target_spiffe_id": "spiffe://rete-lovers/service/mongodb",
...
The two join keys are the **link name** (ctrl `active_links` and every `in.link`/`out.link` ⊆ the mgmt `links` table) and the **flow tuple `(scope, target_spiffe_id, principal_spiffe_id)`** (ctrl `label_bindings` ⊆ the mgmt `ingress`/`egress` allow-set, expanded once). The agent verifies, before applying:
...
3. every `label_bindings (scope, target, principal)` ∈ the expanded mgmt allow-set.
