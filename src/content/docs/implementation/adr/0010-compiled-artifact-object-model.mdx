...
Realises, concretely, the "object model on the consumed side" that [ADR-0009: Treat Source Config as a Compiled Facade](/docs/implementation/adr/0009-config-compiled-facade) placed but did not shape. Uses the rete/mesh vocabulary of [ADR-0008](/docs/implementation/adr/0008-name-rete-mesh-cluster) and the identity-as-key discipline of [ADR-0007](/docs/implementation/adr/0007-decouple-naming-identity-routing). Grounds and amends the artifact schemas in the C0 [Validate & Compile](/docs/implementation/c0-tended-tunnels/mgmt-plane/validate-and-compile) and C1 [Mgmt Plane](/docs/implementation/c1-manual-mesh/mgmt-plane) / [Ctrl Plane](/docs/implementation/c1-manual-mesh/ctrl-plane) docs. Signer-set placement is decided here but its semantics live in [ADR-0015: Anchor Signer Trust in the Root CA](/docs/implementation/adr/0015-anchor-signer-trust-in-root-ca), which replaced the obsoleted ADR-0011.

> **Amendment note (2026-07).** The [C0 Agent design](/docs/implementation/c0-tended-tunnels/mgmt-plane/agent) refined three decisions recorded here, and this record is amended in place accordingly. (1) The supervision inventory carries **no `kind` at all** (a later refinement of the same pass — an earlier state of this amendment had `vertex` / `workload`): the launch recipe was `kind`'s only consumer, and it is now explicit as a per-entry `run` argv, with all per-workload context injected through a uniform spawn environment; the payload's link/mesh engine kind was never in the inventory — the agent parses no payload but its own, so an engine tag there would be dead weight. (2) Artifacts carry **no filesystem references** — `ca_cert_path` / `cert_path` / `priv_path` are gone; identity is keyed by SPIFFE ID and materialized by the node's identity store. (3) The mgmt↔ctrl join is verified by the **vertex** — the only party parsing both planes — not the agent, which verifies envelopes and signatures only; correspondingly, the versioning gate splits into an envelope contract and per-payload-family contracts, each family's version riding inside the payload as its own `schema_version` (see the amended [ADR-0012](/docs/implementation/adr/0012-version-artifact-contract)). (4) The envelope `kind` axis is **removed**: identity is `(node, plane, name)`, `agent` is a reserved name, and dispatch is by name to the consuming workload — consumer kinds never enter the envelope, so a new supervised workload never changes it. The text below is edited to the amended state.
...
The envelope carries one axis: `plane` — whose authority signed this. *Which process consumes this* is answered by `name` alone: dispatch is by name, each consumer parsing only artifacts addressed to it (`agent` reserved for the supervisor), so consumer kinds never enter the envelope and a new supervised workload never changes it. The link/mesh distinction is a **payload** axis:

| Axis | Question | Values | Location |
|---|---|---|---|
| `plane` | Whose authority signed this? | `mgmt` / `ctrl` | envelope |
| `kind` (vertex payload) | Which engine does the vertex run? | `link` / `mesh` | payload |

Naming is fixed by two words used identically in YAML and JSON: **`kind` = the category of an object** (vertex: `link`/`mesh`), **`type` = the variant within a context** (transport: `quic`; adapter: `udp`/`florio`/`wg`; bound rule: `list`). No YAML change: the YAML already uses `kind: link|mesh` and `type: quic`. Because the unified `links` / `egress` / `ingress` / `connection_manager` shapes (below) make the mgmt payload field-identical for both vertex kinds, the payload `kind` is a **plain discriminator field**, not a tagged union with per-variant data; it selects the forwarding engine, gates validation, and decides whether a ctrl artifact is consumed.
...
```json
{
  "schema_version": "1.0", "plane": { "ctrl": { "obeys_mgmt_version": 42 } },
...
The two join keys are the **link name** (ctrl `active_links` and every `in.link`/`out.link` ⊆ the mgmt `links` table) and the **flow tuple `(scope, target, initiator)`** (ctrl `label_bindings` ⊆ the mgmt `ingress`/`egress` allow-set, expanded once). The **vertex** verifies, before applying (it is the only party that parses both planes; the agent verifies envelopes and signatures only):
...
### The agent inventory is supervision-only, and paths use one root

`agent.json` lists `workloads: [ { "name", "run" } ]` — a supervision inventory the agent reads without parsing any other payload. `run` is a plain argv naming the executable; all per-workload context (scope, roots, the verified config's path) is injected through the uniform spawn environment, so the agent applies no per-entry special-casing — a vertex launches exactly like any other program. The link/mesh engine distinction lives solely in the vertex payload, where the engine reads it. There is **no per-workload config path**: configs are located by `name` under fixed per-plane conventions in the scope's artifact root (`<name>.json` in the active mgmt set; `ctrl/<name>@v<K>.json` slots) — never a config-supplied resolution root — and each workload receives its own config's resolved path at spawn. Supervision metadata — the inventory, how the agent reaches the Coordinator (`sync`: its principal and the coordinator identity; the dial-out resolves through the agent's own `inbounds` entry), and the compiler-projected `inbounds` endpoint map (each inbound a typed `{ kind, addr }` so a new inbound protocol doesn't change the schema) — belongs in `agent.json`; engine internals stay in the vertex config.

### `agent.json` carries the ctrl-signer ACL *(revised)*

This section originally placed **two** signer sets in a `trust` block in `agent.json` — `authorized_mgmt_signers` and `authorized_ctrl_signers` — so the rete-wide trust roots sat once where the sole verifier reads them, removing a C0/C1 inconsistency (mgmt signers repeated in every envelope, ctrl signers buried in the mesh vertex payload). [ADR-0015](/docs/implementation/adr/0015-anchor-signer-trust-in-root-ca) removes the first and keeps the second, and the placement decision survives with a better rationale than the one it was made on.

**`authorized_mgmt_signers` is gone.** A mgmt signer's certificate travels **inside the `signature` of the envelope it signed** and is validated against the node's trust anchors at `identity/anchors.pem`, which are enrollment-delivered and referenced by no artifact. A mgmt ACL carried in a mgmt artifact would be verified against the list it is updating, which deadlocks a node absent across a mgmt signer identity change.

**`authorized_ctrl_signers` stays, and `agent.json` is exactly where it belongs** — not for consistency, which was the original reason, but because **a plane may constrain the plane below it and never itself**. `agent.json` is a mgmt artifact, so a ctrl ACL in it never gates its own delivery: an absent node fetches the current mgmt set, verifies it against the anchors, and receives the new ctrl list in the ordinary way. It holds **SPIFFE identities rather than keys**, so a ctrl signer's certificate can be replaced without any artifact changing while withdrawing its authority stays a single mgmt-signed edit.

The rest of this record — the envelope's routing core, payload separation, and the per-artifact placement rules — stands unchanged.

### Envelope naming

Keep `payload` (signed-container convention) — not `spec`, which carries the Kubernetes "user-authored desired state" meaning that belongs to the *facade*, not the compiled projection. The envelope stays flat (claims beside `payload`) and **payload-agnostic**: it carries a generic `name` — the consuming workload's name (`agent` reserved for the supervisor) — so identity is `(node, plane, name)`, never path-derived and never consumer-kind-tagged. The envelope is a closed, `schema_version`-gated schema (unknown fields rejected): it evolves by adding an explicit field under a `schema_version` (`major.minor`) bump, never an open extension bag. A consumer gates each artifact **fail-closed and one-directionally**: it reads its own major at any older-or-equal minor (so a newer flor keeps consuming older artifacts — the rolling-upgrade direction), but **rejects a newer minor** (and any other major) rather than silently ignoring fields it doesn't understand — because `deny_unknown_fields` means a newer minor may have added a restriction, and dropping it would fail *open*. Compatibility across a minor bump is thus achieved by upgrading consumers before the producer emits the new minor, not by lenient parsing. Per-plane metadata rides on the `plane` discriminator itself — `plane` is a tagged value, `"mgmt"` or `{ "ctrl": { "obeys_mgmt_version": … } }` — so a ctrl-only field needs no optional envelope claim, and a payload's per-`kind` data stays in `payload`. [ADR-0012: Version the Compiled-Artifact Contract](/docs/implementation/adr/0012-version-artifact-contract) (as amended) owns the versioning policy: `schema_version` gates the **envelope contract**, while each payload family carries its own `schema_version` inside the payload, gated by that payload's consumer under the same fail-closed discipline — producer stamping, relay obligations, and the minor/major evolution rules apply within each contract.
...
- The vertex's "combine mgmt + ctrl" step is a zip on link names and identities, not an expand-and-match.
...
- `first_hop` moves mgmt → ctrl, a real change from the current C1 docs (which must be amended), and `authorized_ctrl_signers` moves to `agent.json` (see [ADR-0015](/docs/implementation/adr/0015-anchor-signer-trust-in-root-ca); the mgmt set it was once paired with is gone).
