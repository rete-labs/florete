...
1. **Single workload.** The Coordinator is one workload, published as a rete service; the loop runs in-process, Knowledge in-process. (C0's `coordinator` is this, degenerate to store-and-serve — it decides nothing yet.)
...
- Time service: a first rete-side design now exists ([Network Clock](/docs/implementation/b2-hybrid-hauling/network-clock) — Coordinator-referenced, agent-disciplined, bounded); what remains here is consensus time inside a distributed Coordinator and the Time service's northbound API.
...
- **Coordinator placement and the split/join protocol** (needs the Orchestrator). Placement is authored today as a concrete point (`at: mgmt01`) — the degenerate collapse of what is really a decidable dimension, exactly as a complete `spec` is the degenerate collapse of a bounds-plus-objectives space. The general form follows the same refinement chain as everything else here: the operator authors a placement *space* ("this service may run on any of these nodes") plus objectives, the Coordinator picks a point within it, emits it as a ctrl decision, and agents start or stop the local instance. Split/join is the same operation with more than one answer alive at once. Two things follow. First, if flor's binaries are present on every node — an open C0 installer question, not a settled one — then this initial Orchestrator ships **decisions only**: no registry, no image pull, no content-addressed delivery, which is a large simplification over container orchestrators. Second, that property holds precisely because the workloads are *Florete's own components*, few and pre-installed; extending the executor to **user** workloads reintroduces delivery and is a milestone of its own. Starting with the self-hosting case is therefore the cheap first version, not a dead end.
