...
A point the rest of this sketch must respect: **the operator-facing YAML and what the nodes and Coordinator consume are different layers.** The YAML is a *facade* — authored as a whole mgmt view, compiled by `florctl`, signed, and discarded. The **compiled JSON** is the operational unit: what is stored, served, sliced, reconciled, and watched. Categories 1–3 (the facade) are isomorphic at the data level to `kind`/`name`/`spec`, but operationally they are a whole-view document, not a live object store — see [ADR-0009](../../../contributing/adr/0009-config-compiled-facade).

Introducing the Coordinator changes the *producer/consumer* picture, not the facade. Today `florctl compile` emits only **node-targeted** artifacts; with a Coordinator there are **three flows**:
...
3. **Per-node ctrl** — produced by the **Coordinator** within those signed bounds (CP-signed), delivered to agents, verified locally against the mgmt bounds.
...
The config format models **only categories 1–3** — the operator-authored facade. The decision ([ADR-0009](../../../contributing/adr/0009-config-compiled-facade)) is therefore: **keep the facade Docker Compose-style and whole-view**, and put the object model only on the *consumed* side (compiled artifacts + the Coordinator's resource model), where state is actually stored, sliced, reconciled, and watched. The facade is already `kind`/`name`/`spec` in disguise (top-level key = kind, map key = name, value = spec), so it loses no addressability; what it correctly *omits* is the live-apply / `status` / reconcile machinery — and the `apiVersion`/namespace/selector ceremony — that a compiled-away facade neither has nor should imply. No `florctl apply -f` in the mgmt path: it would advertise a live store the security model forbids.
...
|---|---|---|---|
| **Authoring / Ingest** (provisional name; today `florctl publish`) | — | Receives the operator's compiled, signed mgmt view (cats 1–3) and stores/serves it; cannot author or re-sign mgmt. No live `apply` for mgmt. | **Yes — relays cats 1–3** |
