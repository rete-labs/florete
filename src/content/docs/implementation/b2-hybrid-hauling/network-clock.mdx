...
- **The signed floor, again.** What enrollment *left on the node* gives a lower bound on now *before any connection exists* — the `notBefore` of its anchors and of every certificate in its identity store, and the `generated_at` of the newest artifact it holds — enough to reject long-expired material and to pass validity checks in the common case. The bound is the material, never the bundle: a bundle is deleted once consumed, and a first enrollment verifies no signature at all, so the floor a fresh node starts from is trusted on the same out-of-band assumption its anchors are ([ADR-0015](/docs/implementation/adr/0015-anchor-signer-trust-in-root-ca), *Validation is bounded by position*). Every later contribution to the floor is signed, and the bound only rises. This is Roughtime's[^roughtime] coarse-then-tight shape, with the coarse bound derived from material the node already holds instead of from a new protocol.
- **Step once, then slew.** First contact validates the presented identity against the node's trust anchors ([ADR-0015](/docs/implementation/adr/0015-anchor-signer-trust-in-root-ca)) and against the floor-based clock with a relaxed validity window; the first in-bounds time fix may **step** the virtual clock forward once; thereafter strict validation and slew-only discipline apply. The exact relaxed-window semantics are an open question below.
