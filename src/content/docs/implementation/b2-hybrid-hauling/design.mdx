...
1. **Joining.** A node comes into radio range → discovery beaconing over the medium ([Link identity & discovery](../link-contract#link-identity--discovery)) → mesh-vertex mTLS over the new link → the node's **agent reaches the Coordinator** over that fresh link — a *second* mTLS relationship — to sync its config and, for a scheduled MAC, receive its radio provision (slot map / grants). Both ends must already be **enrolled**: B2 admission stays operator-driven — discovery *finds* enrolled peers, it never admits unknown ones. That last hop — a newcomer reaching the Coordinator over a neighbor that must forward for it — is trivial in a stable rete but an open problem under mobility ([below](#open-questions)).
...
- **Newcomer-to-Coordinator reachability under mobility.** A joining node's agent↔Coordinator connection is carried over the neighbor it just linked to, which must forward it into the mesh — and a mobile newcomer may have no prior path to the Coordinator at all, unlike the stable C0/C1 rete where the enrollment bundle already encodes the way in. This is an **admission** problem, not a time-sync one — shared with [R6 discovery](../link-contract#link-identity--discovery) and the future [Introductor](../glossary#introductor) — but the [Slot Clock's join sequence](../slot-clock#hearing-vs-exchanging) is where it first bites. Candidate directions live with that page's [open questions](../slot-clock#open-questions): the beaconer/Introductor delivering a *minimal* bootstrap state the newcomer joins on before opening a full Coordinator connection, and degraded contention-slot operation until grants arrive.
- Per-page open questions live with their topics: [Link Contract](../link-contract#open-questions), [Radio MACs](../radio-macs#open-questions), [Slot Clock](../slot-clock#open-questions), [Network Clock](../network-clock#open-questions).
