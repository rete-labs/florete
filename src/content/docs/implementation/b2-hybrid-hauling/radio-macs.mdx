...
How radio MAC models map onto Florete's control architecture. Companion to the [Link Contract](../link-contract) — this page covers its resource-contract requirements R8–R9 and the [RRA](../glossary#rra) extension. Terms: [Glossary](../glossary).
...
Where the split between link vertex and radio driver falls — in particular, who owns the fast-path scheduler state — is radio-specific and one of the main open designs. The mesh layer follows the same pattern at its own level: path failover on liveness loss is a *mesh-vertex* decision, not an agent one.
...
| NR sidelink Mode 1 / Mode 2[^sidelink] | 3GPP's device-to-device mode: in **Mode 1** the network assigns radio resources; in **Mode 2** devices select them autonomously | Both MAC families in one standard — and Mode 2 shows local selection surviving loss of the network controller, the degradation property R9's fast/slow split enables |
...
- The ctrl-artifact schema for radio decisions (power caps now; grants, slot maps, beam schedules later — a `radio` payload alongside forwarding state?).
- Time: source of synchronization, Time service ↔ TDMA clock relationship (R8).
- Where the vendor boundary sits on a FAPI-like split: vendor PHY + whose MAC?
...
[^sidelink]: [3GPP TS 38.300](https://www.3gpp.org/DynaReport/38300.htm) (NR overall description) covers sidelink resource-allocation Modes 1 and 2; the LTE ancestors are V2X Modes 3 and 4.
