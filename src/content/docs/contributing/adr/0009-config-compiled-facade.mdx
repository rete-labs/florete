...
Grounds the source configuration format in [source-layout.mdx](/docs/implementation/c0-tended-tunnels/mgmt-plane/source-layout) and the C0/C1 mgmt-plane docs. Uses the rete vocabulary of [ADR-0008: Distinguish Rete, Mesh, and Cluster](/docs/contributing/adr/0008-name-rete-mesh-cluster) and the bounds/decisions split of the C1 [Ctrl Plane](/docs/implementation/c1-manual-mesh/ctrl-plane). The consumer analysis that motivates it is the [Coordinator sketch](/docs/implementation/b1-cloud-control/coordinator).
...
Mgmt is author → compile → sign → publish, locally. The Coordinator stores and serves mgmt (and can't tamper — nodes verify the operator signature) but cannot author it. Offering `florctl apply -f` for mgmt would advertise a live-object model that does not exist and that the blast-radius design rules out. On-prem deployments may relax this (a scoped mgmt key on the Coordinator); cloud deployments do not.
...
- `florctl apply -f` may appear later for *non-mgmt* or on-prem-relaxed paths once a reconciling target exists; it is not the mgmt model.
