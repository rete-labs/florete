...
Refines the naming/SNI mechanism of [ADR-0006: Implement mTLS in QuicEndpoint](/docs/contributing/adr/0006-impl-mtls-in-quic-endpoint). The *rete* / *mesh* / *cluster* vocabulary this ADR uses is defined in [ADR-0008: Distinguish Rete, Mesh, and Cluster](/docs/contributing/adr/0008-name-rete-mesh-cluster).

## Context

[ADR-0006](/docs/contributing/adr/0006-impl-mtls-in-quic-endpoint) settled mTLS and pinned the QUIC SNI to the `.rete` *convenience hostname*: the server was to read SNI, run `dns::parse` to recover a `SpiffeId`, and look up which published cert to present. [identity.mdx](/docs/implementation/c0-tended-tunnels/mgmt-plane/identity) defines that hostname as `<service>.<rete>.rete` (rete-scoped) or `<service>.<node>.<rete>.rete` (node-scoped), with `<rete>` standing in for the SPIFFE trust domain.
