...
Florete uses SPIFFE IDs as cluster-internal principal identities (see [identity.mdx](/docs/implementation/c0-tended-tunnels/mgmt-plane/identity)). The `flor` crate has no Rust types for these yet — the QUIC endpoint runs on a self-signed cert with a trust-all client verifier. Before we can implement mTLS, certificate operations in `florctl`, or any of the management/control-plane artifact flows, we need a typed identity model and a CA primitive.
...
- how cluster signing authorities (control-plane, mgmt-plane) relate to the CA,
...
pub enum Scope { Cluster, Node(String) }   // node-name segment, not a SpiffeId
```
...
`SpiffeId` stays the canonical name throughout the codebase; `Kind` and `Scope` are computed at points where they matter (e.g. `Ca::sign_csr` deciding the X.509 extension policy, `florctl ca sign` validating `--kind` matches the constructed URI).
...
### Workload credentials struct

Transport, ACL, and CLI all need a single value to carry the local principal's credentials and the trust set used to verify peers:

```rust
pub struct WorkloadCredentials {
    pub trust_bundle: Arc<X509Bundle>,
    pub local: HashMap<SpiffeId, X509Svid>,
}
```

`trust_bundle` is shared across all consumers in the process (inbound verifier, outbound verifier). `local` maps each local principal's SPIFFE ID to its full credential. A vertex hosting multiple principals — `node/alpha` plus a couple of services plus user identities — has multiple entries; each outbound `connect` call looks up the *caller's* credential here, and the inbound server-cert resolver picks based on SNI.

The struct lives in `src/core/identity/credentials.rs`. SPIFFE-native naming ("workload credentials" is the SPIFFE term for "what a workload owns"); short to type; doesn't pretend to be a generic abstraction.

**`WorkloadCredentials` is mTLS-only.** Envelope verification (mgmt and ctrl) needs a separate kind of trust storage — pinned signer pubkeys, not chain-validation authorities — and a separate set of consumers (the artifact loader, not the transport). When the envelope-verification follow-up lands, it introduces its own type (something like `EnvelopeTrust`); we deliberately do *not* extend `WorkloadCredentials` with envelope-verification fields. The CA happens to be the issuer for both mTLS leaves and envelope-signer certs, but the runtime types those certs feed into are distinct because the verifier flows are distinct (`webpki::verify_for_usage` for one, ed25519 against an expected/pinned pubkey for the other).

### Single CA, no intermediate, in C0/C1

A `Ca` value represents a self-signed cluster root. All six kinds are issued through the same `Ca::sign_csr(csr_pem, &SpiffeId, Kind, validity)` path — the `Kind` argument selects the X.509 extension policy. Control-plane and management-plane signing identities are *regular principals under the CA* (with restricted X.509 extensions), not intermediate CAs.
...
This ADR's runtime scope is mTLS — the cert chain authorities (in `WorkloadCredentials.trust_bundle`) and local workload SVIDs (in `WorkloadCredentials.local`). Envelope verification implementation lands in a follow-up alongside mgmt/ctrl artifact loading.
...
4. **Use `SpiffeId` directly.** **Accepted.**
	...
	Pros: zero abstraction tax; the upstream `SpiffeId` API is what we'd want anyway; the codebase reads as what it is — a SPIFFE-only mesh transport.
...
- `florctl ca sign --kind control-plane` should work in C0 even though no envelope signing happens yet, because the CLI is the user-visible contract and adding a flag value later is more disruptive than starting wide.
...
The earlier florete spec used `"key_id": "operator:<fingerprint>"` for mgmt envelopes — the operator's signing key wasn't a SPIFFE principal at all, it was a raw fingerprint string. Meanwhile control-plane signatures already used a proper SPIFFE URI (`spiffe://<cluster>/control-planes/<name>`). The two should be symmetric:
...
- If non-SPIFFE identity classes (DNS for ingress GW, `.onion`) eventually arrive *and* turn out to share verifier code with the SPIFFE mesh, we will refactor at that point. We accept the refactor cost as the price of not abstracting prematurely.
