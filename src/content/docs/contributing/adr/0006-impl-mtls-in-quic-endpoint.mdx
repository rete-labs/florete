...
- **Identity primitives** under `src/core/identity/` per [ADR-0005](/docs/contributing/adr/0005-use-spiffe-identities) — re-exports `spiffe::SpiffeId` / `spiffe::TrustDomain`, projects `Kind` / `Scope`, provides `Ca` and `keygen_csr`.
...
- validate both directions against the cluster's `X509Bundle`,
...
- `spiffe://<cluster>/service/<svc>` → `<svc>.<cluster>.rete`
- `spiffe://<cluster>/service/<node>/<svc>` → `<svc>.<node>.<cluster>.rete`
- `spiffe://<cluster>/vertex/<node>/<name>` → `<name>.<node>.<cluster>.rete` (C1)
...
**Service and vertex share the same DNS namespace.** A vertex on node `alpha` named `cluster` produces `cluster.alpha.<cluster>.rete`; a service on node `alpha` named `cluster` would produce the same string. The cluster compiler/validator enforces that service and vertex names on the same node don't collide (extending the existing identity.mdx rule that already forbids node↔service name collisions). Keeping the namespace bare — no `svc` / `vrt` infix — is justified by:
...
Internally, `connect` calls `Resolver::resolve(target) -> SocketAddr` to find where to dial, derives the SNI via `dns::format(target)`, builds a per-call rustls `ClientConfig` with the caller SVID and a `SpiffeServerVerifier` parameterised by the expected target, and dials.
...
- `SpiffeResolvesServerCert` — `rustls::server::ResolvesServerCert` impl. Reads the SNI as a `.rete` convenience name, parses it back to a `SpiffeId` via `dns::parse`, returns the matching SVID from the endpoint's published-services table (populated by `QuicPublisher::publish`). A miss (unknown SNI, unknown SPIFFE ID, nothing published for it) means no cert is presented and the handshake fails — that's the right outcome.
- `SpiffeClientCertVerifier` — validates the inbound peer cert chain against the cluster's `X509Bundle` (using `webpki` directly or `WebPkiClientVerifier` if it composes cleanly). The leaf's URI SAN is what authenticates the peer; surfaced to the acceptor via `peer_identity()` and returned as the `SpiffeId` in `accept`.
...
- `handle_socks5` parses the SOCKS5 target host (e.g. `api.alpha.cluster.rete`) via `dns::parse(host) -> SpiffeId` instead of forwarding it as a raw string, then calls `connector.connect(&self.caller_svid, &target_spiffe).await`.
...
1. **K8s-style infix** (`api.alpha.svc.cluster.rete` vs `api.alpha.vrt.cluster.rete`). Unambiguous. Adds boilerplate to every user-typed `.rete` name; people grumble about this in K8s already.
...
- chain-validating against the cluster's bundle — yes,
...
- When B1+ introduces an intermediate CA (per [ADR-0005](/docs/contributing/adr/0005-use-spiffe-identities)), the trust bundle already supports multi-authority sets; webpki already validates multi-element chains. No verifier-API rework needed.
