...
Do not hard-wrap prose: one paragraph is one line. Editors soft-wrap at whatever width the reader is using, and embedded newlines make that view ragged and every later diff noisier than the edit it carries. Fixed-width blocks are the exception and belong inside a fence — use ```` ```log ```` where blank lines carry grouping, since a bare fence renders condensed.

### Style

No emoji. Use bullet lists and emphasis sparingly — reach for them when a list is genuinely a list, not to break up prose that reads fine as prose. Keep your own voice: a page in your words beats a smoother one in nobody's.
