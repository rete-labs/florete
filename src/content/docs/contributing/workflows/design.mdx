...
What follows is a pass with **four sign-offs**. At each, work stops until you agree:
...
3. **Verdicts** — one pick per axis with its reasoning, in a workspace file you comment on by ID.
4. **Record** — the skeleton written into the page. You check the render and choose: prose now, or stop.

Prose is optional and takes no gate, but the pass still needs an end: you say the prose is finished, and the **implementation handoff** is written then. Stopping at record instead emits a **prose handoff**, so a later session can pick the writing up.
...
```
  ...
  produces    one design page, in two halves, plus two handoff notes
...
```
  ...
  RECORD    the workspace compacted into the page's skeleton
       → GATE 4  clears when it renders, and the human picks
                 prose here, or a stop with the prose handoff
  PROSE     orient · surface · internals — no gate, optional
            ends when the human calls it done, with the implementation handoff
```
...
**Greenfield** sketches a shape whose job is to be contested — it asserts things it has not earned, so their silences become visible. **Revision** reads the existing skeleton and amends it; there is no sketch, because the shape already exists and has a stricter quality bar. **Curation** builds a skeleton where prose exists but none does, harvesting decisions the prose already made implicitly rather than making new ones.
...
**A gate is a state, not a message.** It clears on a condition, and questions, redraws and amendments happen *inside* it — a gate is not one exchange. The conditions are: the merged axis list is agreed; the candidates are agreed; every axis is ratified; the page renders and the human picks what follows.
...
Two notes leave a pass, and they are chained:

- The **prose handoff** is emitted at gate 4 only when prose is deferred. It carries what prose needs and the log deliberately drops — findings, deliberate deferrals, cross-page effects, and the framings that emerged in argument without becoming decisions — plus the implementation notes gathered so far, which would otherwise be lost between sessions.
- The **implementation handoff** always ends the pass. It carries the plan-shaped material the skeleton refuses: migration steps, current-state facts, ordering, anything answering *how do we get from here to there*.

When prose is deferred, the prose session receives the first note and emits the second. When prose runs in the same session there is one note, at the end.
...
The agent **creates** a form — its heading, its structure, and one worked example of the shape being asked for — and then never touches what the human writes into it. The options form arrives already carrying the agreed axis list, so the human fills rather than transcribes.

Four files, in `design-wip/<ticket>-<topic>/` in the repository holding the target page, which git ignores. The directory carries the pass's identity, so the files need no prefix. A ticket number is written bare when it belongs to that repository and prefixed with the repository name when it does not — `flor-81-design-change` — the form [topic branches](../git-workflow#topic-branches) already use.

| File | Medium | Holds |
|---|---|---|
| `axes.md` | form | the merged axis list |
| `options.md` | form | candidates, criteria and rank |
| `shape.md` | workspace | the shape, from first read to record |
| `verdicts.md` | workspace | one entry per axis, open and closed |

The **shape file** emerges when the target is read and is amended as decisions land; it is never regenerated from the verdicts. The **verdicts file** shows a counter, an open group and a closed group below a separator; closing an item compacts it to its question, verdict, one-clause reason and its rejections — the same compression record performs, so it converges toward the log as the pass proceeds.
...
*Rejected* top-of-page placement, premised on "read this first", false once the audience is named; a collapsed block, since a designer would rather jump to a section than uncover one, and a cut can break in-page search; duplicating the shape in both places, which is a sync burden for a formatting gain.
...
At gate 4 the human picks: **prose here**, in the context that made the decisions, or **stop**. Prose is not derivable from the log — the log is the compressed subset by construction. **Review is a third thing**, never inside the pass: it runs in fresh context and takes the **finished page**, skeleton and prose together, plus whatever the review skill supplies. It consumes no handoff.
...
*Rejected* gating prose inline, which puts it in front of the one reviewer least able to judge it; feeding the handoff to review, which confused a writing aid for a reviewing input.

**DESIGN-08. Two handoffs leave a pass, and they are chained.**
The **prose handoff** is emitted at gate 4 only when prose is deferred, carrying what prose needs and the log drops — findings, deliberate deferrals, cross-page effects, framings that never became decisions — plus the implementation notes gathered so far. The **implementation handoff** always ends the pass, carrying the plan-shaped material the skeleton refuses: migration steps, current-state facts, ordering. When prose is deferred, the prose session receives the first and emits the second; when prose runs in the same session there is one note, at the end. Prose takes no gate, but the pass needs a trigger for that last note: the human calls the prose done. That is a **completion signal, not a review** — judging prose is review's job, in fresh context.
*Because* the drop criterion rejects implementation and plan material, and a filter that only rejects loses information. Routing it gives those decisions a destination instead of a bin — this design misfiled a migration step into internals prose precisely because none existed.
*Accepting* a deferred prose session must carry material it does not itself use.
*Rejected* one handoff for both readers, which would hand an implementer the framings and an explanation of what prose still owes; discarding the plan, which is real work done during the pass.

**DESIGN-09. Four sign-offs, and stages add capability without removing it.**
Gates sit after axes, options, verdicts and record. A gate is a state rather than a message: it clears on a condition, and questions, redraws and amendments happen inside it. A stage gates what may be *started*, never what may still be *changed*. Stance is a gradient **shared by both parties** — Challenger early, Decider late — which gives convergence a definition: the decision work is converged when there is nothing left to challenge. What ends the *pass* is the human's choice at record, or their word that the prose is finished.
...
*Rejected* a single cross-milestone sequence, which needs a register; path-derived or page-scoped IDs, which break on a move or lose topic identity; a dot separator, which breaks the `ADR-0011` house convention for a marginal gain; freezing IDs from the first draft, which would strand a late entry away from its topic for the life of the page.
