...
Two things about how you engage. **The stance shifts**: early on you are hunting for what is missing, late on you are judging picks — and the pass ends when there is nothing left to challenge. And **the sequence is a default, not a rule**: arriving with your own axes and options already written is a normal entry point, and the skill picks up wherever you are.

Working files land in a scratch directory outside any repository. The skill reports them at the end and never deletes them unasked.
...
Four files, in a scratch directory outside any repository:

| File | Medium | Holds |
|---|---|---|
| `<topic>-axes.md` | form | the merged axis list |
| `<topic>-options.md` | form | candidates, criteria and rank |
| `<topic>-shape.md` | workspace | the shape, from first read to record |
| `<topic>-verdicts.md` | workspace | one entry per axis, open and closed |
