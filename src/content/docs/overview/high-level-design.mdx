...
Florete technology is built of two intertwinted parts: the Florenetes orchestrator and the Florete network. The orchestrator runs over the Florete network, which provides a service mesh abstraction. The network's control plane is itself a distributed app managed by some orchestrator. Inter-cluster networking is implemented by Florete network as well, extended beyond in-cluster service mesh.

<Callout title="Rete, and its (future) relationship to clusters">
We name a single Florete network's trust domain — one CA, one identity/principal namespace, one boundary within which names resolve and peers authenticate — a [rete](#rete); the `.rete` hostname TLD names it. Where this document says "in-cluster service mesh" or "cluster layer", read the *network* boundary as a rete.

How a rete maps to a **Florenetes cluster** (the orchestrator unit, [below](#florenetes-design)) is **not yet designed**. It may be one-to-one — or, by analogy with an Istio mesh spanning several Kubernetes clusters, a single rete may **span multiple clusters**. The abstract model of Florete is independent of that mapping. One consequence to keep in mind as the design firms up: **interrete** is precisely *inter-rete* (between trust domains), which — once a rete can span clusters — is distinct from inter-cluster connectivity *within* a single rete. We keep this open until the cluster concept is designed.
</Callout>
...
The model is small: three **abstractions** — layer, vertex, service — describe any layer; each layer is one of two **kinds** (a link or a mesh); and two conventional overlays (a rete, an interrete) mark the **trust boundary** a mesh spans.

### Abstractions

The core nouns every Florete layer is described with — independent of how the layer is implemented.
...
In classical naming terms, a vertex is a **network attachment point**: the point where a layer's users attach to the layer — Saltzer's *point of attachment*[^poa], the same structure RINA reaches with its layer-internal names. The notion recurses with the architecture: a vertex's own attachment points are the lower-layer vertices it uses. Saltzer's chain *service → node → attachment point → route* thus maps onto Florete as *workload identity → mesh-vertex → used vertex → path*; and any wire-level address a medium may need is a layer-private label naming an attachment point — never a name of the layer's user (see [Addressing & Routing](#addressing--routing)).

[^poa]: J. Saltzer, [RFC 1498 — On the Naming and Binding of Network Destinations](https://datatracker.ietf.org/doc/rfc1498/): the canonical *service name → node → point of attachment → route* binding chain, each step resolved by a directory.
...
Let's define Florete network **service** as an entity that is published (via Portal::Publish) in some layer. This is an abstraction of (micro)services in a service mesh. Any vertex from a recursive layer (a [mesh](#mesh)) is a service, because it publishes itself in some other layers. Note that vertices of a [link](#link) are not services themselves, because there is no other layer to publish them.

### Layer Kinds

A layer — and the vertex that implements it — is one of two kinds: a primitive **link**, or a recursive **mesh**.
...
Opposite the link sits the general, recursive layer — a **mesh**. A mesh is a service mesh in the full sense: a fabric where any user can reach any other ([the reason](#layer) we call a layer a mesh), built not on a physical medium but on the [Portal](#vertex) APIs of *other* layers. This is where the recursion of the architecture lives.

A mesh is implemented by a **mesh-vertex** — the non-degenerate [vertex](#vertex), with all three parts active: a transport endpoint terminating connections for Portal users, a connection manager holding in-layer connections to peer mesh-vertices (carried over lower layers), and a non-trivial **forwarding engine** that moves packets by labels (MPLS-style) — local-origin, local-destination, and transit.

So **link** and **mesh** are the two shapes a layer — and the vertex implementing it — can take: the link is the primitive base case (exactly one peer, no transit); the mesh is the recursive case (many peers, label-forwarded transit). Above the physical medium, every layer is a mesh, built recursively on links and on other meshes.

### Trust Boundaries

Beyond its kind, a mesh sits on a second axis — the **trust boundary** it spans. Two conventional overlays, named after the scopes they cover, mark its extremes.

#### Rete

A **rete** is a trust domain: a set of nodes sharing one CA and one identity namespace — the boundary within which names resolve and peers authenticate. The base [mesh](#mesh) over a rete's nodes is, by convention, formed of mesh-vertices named `rete`, because that mesh *spans* the rete.

<Callout title="The rete (trust domain) vs. the mesh named after it">
Do not read "rete" as "the first mesh overlay". The **rete** is the *trust domain* — an identity and security boundary. The first mesh overlay is a **mesh** — one of potentially several a node joins — that happens to span that rete and is therefore *named* `rete` after it. The naming is a convention, not an identity: the mesh spans the rete; it is not the rete.
</Callout>

#### Interrete

An **interrete** is a *federation* of retes — several trust domains interconnected. Similarly, a [mesh](#mesh) layered on top of the per-rete meshes and links is called interrete mesh (its mesh-vertices are named `interrete`). It is spanning trust-domain boundaries rather than living inside one. Where a rete mesh carries traffic *within* a trust domain, an interrete mesh carries it *between* them — trust **federated** across the participating retes, never silently shared. Exploratory notes on the global case are gathered under [Interrete Thoughts](#interrete-thoughts).
...
What about addresses? That concept becomes private implementation detail of a particular layer. Where a layer does need them, addresses label **attachment points** — the [vertices](#vertex) its users attach through — never the users themselves; user names bind to attachment-point labels per layer, by a directory or by derivation. Actually, for small networks there is no need for addresses at all: names are enough, and routing can be done by other methods. The most well-known alternative to IP routing is MPLS[^mpls]. It enables Traffic Engineering[^te] — the thing we need too. MPLS uses labels instead of addresses for forwarding packets.
...
Interrete is inter-domain networking (lowercase when referencing just some inter-rete layer; capitalized when referencing a global network of retes). It is built using the same recursive layering principle as the Florete network of the rete (service mesh).

Interrete connectivity can be provided similarly to IP network services, just for native Florete network. Probably there will be native Florete Ingress and Egress Gateways — distributed services that implement interrete connectivity via remote Portal APIs. Alternatively, it is possible to add interrete connectivity via local proxies over the rete layer. The implications of these different approaches need to be investigated.

Interrete is a layer of Florete network that isn't centrally managed. There are a few interconnected retes that manage their own segments of the layer. And there are borders where they're actually connected.
...
- There will be much more user retes than ASes now, as that's not prerogative of ISPs and large corporations
- Operating a rete connected to Interrete should be fully automated and not require technical skills of the owner
- Dynamic routing between retes must support high mobility of nodes — it would be normal for connection points to change
